...
namespace Controllers {
    ...
    class RequestHandler extends RequestAbstract {
        ...
        public function __construct($request, $origin) {
            ...
            if ($this->endpoint == 'qugeoregistration' || $this->endpoint == 'collectregistration')
            {
            ...
            elseif ($this->endpoint != 'auth' || $this->verb != 'auth' || $this->method != 'GET')
            {
                ...
                {
                    //print_r($_REQUEST);
                    throw new \Exception('No Order ID Key provided ' . json_encode($this->request) . $this->endpoint . ' ' . $this->verb . ' ' . $this->method);
                }
                elseif (!$auth->verifyKey($this->request['orderid'], $origin, $response))
                {
        ...
        protected function wallet() {
            $wallet = new Wallet($this->method . '_' . $this->verb);
            return $wallet->{$this->method . '_' . $this->verb}($this->args, $this->request);
        }
