...
class User {
    ...
    private function validateBranchOffice(){
        return true;
    }
    ...
    private function checkUserTypeIntegration($User_type, $user_companies, $user_branches) {
        ...
        switch (intval($finascop_user_type)) {
            ...
            case 4:
                ...
                break;
            case 5:
                    if (!$this->validateBranchOffice($user_branches)) {
                        echo "{success:false,errors:'FINASCOP:Received Invaild branch office as parameter.' }";
                        exit();
                    }
                    break;
            default:
    ...
    private function configureUserComapanyAndBranch($User_type, $cmp_br_relations, $UserId) {
        ...
        $db->query("DELETE from " . FINASCOP_DB . "`finascop_user_branchoffices` WHERE `UserId` = {$UserId}");
        ...
        switch ($finascop_user_type) {
            ...
            case AUDITOR:
                ...
                break;
            case BRANCH_OFFICE:
                $branch_office = $cmp_br_relations['user_branch'];

                if (!empty($branch_office)) {
                    //$active_branch_arr = explode(',', $active_branch);

                    foreach ($branch_office as $value) {
                        $user_activebranches['boId'] = $value;
                        $user_activebranches['UserId'] = $UserId;
                        $db->perform(FINASCOP_DB . 'finascop_user_branchoffices', $user_activebranches);
                    }
                } else {
                    echo "{success:false,errors: { reason: 'FINASCOP:Please add user branch offices ' }}";
                    exit(1);
                }
                break;
            default:
    ...
    public function additionalLoginActions(&$SESSION, $IsAppLogin, $user_id, $typid) {
        ...
        switch ($SESSION->finascop_typId) {
            ...
            case 5:
                $qry = "SELECT count(*) as cnt from " . FINASCOP_DB . "finascop_company 
                WHERE  comp_id IN( SELECT boCompany  from " . FINASCOP_DB . "branch_office 
                WHERE id IN(SELECT boId from " . FINASCOP_DB . "finascop_user_branchoffices WHERE UserId = {$user_id})) ";
                $SESSION->UserType = "Branch Office";
                    break;
            default:
                $qry = "SELECT 0 as cnt ";
    ...
    public function getUserActiveComapnies($Finascop_typId,$Finascop_UserId) {
        ...
        switch ($Finascop_typId) {
            ...
            case 5:
                    $qry = "SELECT comp_id AS id, comp_name AS `name` from " . FINASCOP_DB . "finascop_company 
                    WHERE  comp_id IN( SELECT boCompany  from " . FINASCOP_DB . "branch_office 
                    WHERE id IN(SELECT boId from " . FINASCOP_DB . "finascop_user_branchoffices WHERE UserId = {$Finascop_UserId})) ";
                    break;
            default:
                $qry = "SELECT comp_id AS id, comp_name AS `name` from " . FINASCOP_DB . "finascop_company 
				WHERE  comp_id IN( SELECT comp_id  from " . FINASCOP_DB . "finascop_branch_company ) ";
    ...
    public function getUserActiveBranches($Finascop_typId,$Finascop_UserId,$CompanyId,$allbranch=false) {
        ...
        switch ($Finascop_typId) {
            ...
            case 3:
                ...
                break;
            case 4:
                $qry = "SELECT distinct finascop_branch.br_ID AS id, CONCAT(finascop_branch.br_Name,' - ',finascop_branch.branch_shortname) AS `name` from " . FINASCOP_DB . "finascop_branch inner join " . FINASCOP_DB . "finascop_branch_company on finascop_branch_company.br_Id = finascop_branch.br_Id "
                ...
                break;
            case 5:
                $qry = "SELECT id,boName AS `name` from branch_office WHERE id IN(SELECT boId from " . FINASCOP_DB . "finascop_user_branchoffices WHERE UserId = {$Finascop_UserId}) ";
                break;
            default:
                $qry = "SELECT br_ID AS id, CONCAT(br_Name,' - ',branch_shortname) AS `name` from " . FINASCOP_DB . "finascop_branch"
                        . " WHERE br_status = 'Active' AND br_ID IN (SELECT DISTINCT br_Id from " . FINASCOP_DB . "`finascop_branch_company` WHERE 1=1  {$compidstr} )";
