    ...
    class Branch
    {
        ...
        public function saveBranchGroup($inputdata, &$return_rec)
        {
            ...
            $data = array(
                ...
                "exclusiveMerchant" => $inputdata['exclusiveMerchant'],
                "reservedMerchant" => $inputdata['reservedMerchant'],
            );
            if(!empty($inputdata['mobile']))
                $data['contactNumber'] = $inputdata['mobile'];
            if ($inputdata['store_group_outofStock'] == 'true') {
                $data['store_group_outofStock'] = 1;
            } else {
                $data['store_group_outofStock'] = 0;
            }
            if ($inputdata['store_group_grosmartMerchant'] == 'true') {
                $data['store_group_grosmartMerchant'] = 1;
            } else {
                $data['store_group_grosmartMerchant'] = 0;
            }
            if ($inputdata['isFeatured'] == 'true') {
                $data['isFeatured'] = 1;
            } else {
                $data['isFeatured'] = 0;
            }
            ...
            if ($lastId > 0) {
                ...
                if ($data['store_group_grosmartMerchant'] == 0) {
                    $fsbi['issponsered'] = 0;
                    $fsbi['discount_selling_price'] = 0;
                    $fsbi['updated_on'] = date('Y-m-d H:i:s');
                    $branches = $db->getItemFromDB("SELECT GROUP_CONCAT(br_ID) FROM finascop_branch WHERE br_storeGroup = {$lastId}");
                    $status = $db->perform("finascop_stock_branch_inventory", $fsbi, 'update', "branch_id IN ({$branches})");
                }
            }
        ...
        public function saveBranch($data, $IsAddNew, $IsFinascopIntegration = true, &$return_brid = 0)
        {
            ...
            if ($data['br_ID'] > 0 && $IsAddNew == false) {
                ...
                $brKeyCount = $db->getItemFromDB("SELECT IFNULL(MAX(br_key),0) FROM finascop_branch");
                if ($brKeyCount == 0) {
                    $data['br_key'] = 1000;
                } else {
