...
class simpleExcelWriter
{
    ...
    public function exportSupport($query, $heads, $fields){
        global $supportdb;
        $this->checkTotalValues();
        $rs = $supportdb->query($query);
       // print_r($rs);exit();
                
        if($supportdb->num_rows($rs) > 0 ){
	  if($this->writeFile !== false){
	     ob_start();
	  }
          $this->header($heads);
          while ($row = $supportdb->fetch_array($rs)) {
            echo '<tr>';
            foreach($fields as $key){
               $this->cell($row[$key]);
               $this->updateTotalValues($key, $row[$key]);
