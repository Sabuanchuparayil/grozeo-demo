  ...
  var purchaseEntryStore = function () {
    var store = new Ext.data.GroupingStore({
      ...
      reader: new Ext.data.JsonReader({
        ...
        {name: 'fpe_netSgst', type: 'float'},{name: 'totalGst', type: 'float'}]),
      sortInfo: {
        field: 'fpe_id',
        direction: "DESC"
      },
        ...
        buttons: [{
            ...
            handler: function () {
              ...
              if (ft <= (parseInt(record.data.fpod_itemqty) - parseInt(record.data.fpod_invoicedqty))) {
                ...
                var offrrateetch = Math.round((offrrateetch + Number.EPSILON) * 100) / 100;

                var amtBfrTax = invQuantity * offrrateetch;

                amtBfrTax = Math.round((amtBfrTax + Number.EPSILON) * 100,2) / 100;
                ...
                var amtAfrTax = Math.round(amtBfrTax + itemIgst + itemSgst + itemCgst);
                var shortCut = 1 + gstTax / 100;

                amtBfrTax = parseFloat(amtAfrTax/shortCut).toFixed(2);                
  ...
  var invoiceItemViewGrid = function (fped_fpeId) {
    var PurchaseInvoiceItemStore = function () {
      var itemStore = new Ext.data.JsonStore({
        ...
        fields: ['fped_fpodItemName', 'fped_qtyInvoiced', 'fped_igstAmt', 'fped_cgstAmt', 'fped_sgst', 
        'fped_amtBfTax', 'fped_amtAfTax','totalTax'],
        totalProperty: 'totalCount',
        root: 'data',
        remoteSort: true,
        autoLoad: false,
