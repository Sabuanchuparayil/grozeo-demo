...
switch ($op) {
    case 'getGroupDetails':
        $groupId = $db->sanitizeInt($_POST['id']);
        $qry = "SELECT GroupName AS group_name, NatGroupID AS id, "
                ...
                . "from " . FINASCOP_DB . "finascop_accounts_groups ag WHERE Group_ID = {$groupId} ";
        $data = $db->getFromDB($qry, true);
        echo '{"success":true,"data":' . json_encode($data) . '}';
        break;
    case 'saveGroup':

        $group = $db->sanitizeString($_POST['group_name']);
        $nogID = $db->sanitizeInt($_POST['natureOfGroup']);
        $shortCode = $db->sanitizeString($_POST['group_shortname']);
        $groupId = $db->sanitizeInt($_POST['id']);

        if ($groupId == 0) {
            $qry = "SELECT COUNT(1) from " . FINASCOP_DB . "finascop_accounts_groups WHERE UPPER(GroupName) = UPPER('{$group}')";
        } else {
            $qry = "SELECT COUNT(1) from " . FINASCOP_DB . "finascop_accounts_groups WHERE UPPER(GroupName) = UPPER('{$group}') AND Group_ID <> {$groupId}";
        }
    ...
    case 'listGroups':
        ...
        if (isset($_POST['filter'])) {
            ...
            $filterCon .= buildSafeFilterQuery($_POST['filter'], $allowedFields, $db);
        }
