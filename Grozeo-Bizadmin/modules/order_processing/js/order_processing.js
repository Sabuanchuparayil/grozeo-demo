...
Application.OrderProcessing = function () {
    ...
    var ordergridSelectionChanged = function (sm) {
        ...
        if (!Ext.isEmpty(Ext.getCmp('manage_order_grid').getSelectionModel().getSelections())) {
            ...
            var orderStatus = Ext.getCmp('manage_order_grid').getSelectionModel().getSelections()[0].data.StatusId;
            /*if (orderStatus >= 4) {
             Ext.getCmp('rsPrintInvoice').show();
             } 
             if (orderStatus >= 9) {
             Ext.getCmp('rsPrintPackingSlip').show();
             }*/
    ...
    var ordergridSelectionChangedb2cso = function (sm) {
        ...
        if (!Ext.isEmpty(Ext.getCmp('manage_order_gridb2cso').getSelectionModel().getSelections())) {
            ...
            var orderStatus = Ext.getCmp('manage_order_gridb2cso').getSelectionModel().getSelections()[0].data.StatusId;
            /*if (orderStatus >= 4) {
             Ext.getCmp('soPrintInvoice').show();
             } 
             if (orderStatus >= 9) {
             Ext.getCmp('soPrintPackingSlip').show();
             } */
    ...
    var manageOrderStore = function () {
        ...
        var store = new Ext.data.GroupingStore({
            ...
            reader: new Ext.data.JsonReader({
                ...
                'dispatch_courier', 'dispatch_consignment', 'delivery_at_date', 'delivery_at_time', 'delivery_notes', 'StatusId', 'br_Name', 'order_HasReturn', 'order_ItemsReturned', 'order_ReturnVerified'
            ]),
            sortInfo: {
...
//                    printPackingSlip(record.get('order_id'), record.get('order_order_id'));
                    ...
                    handler: function () {
                        ...
                        var order_id = Ext.getCmp('manage_order_grid').getSelectionModel().getSelections()[0].data.order_id;
                        var order_order_id = Ext.getCmp('manage_order_grid').getSelectionModel().getSelections()[0].data.order_order_id;
                        if ((record.StatusId >= 9) && (record.StatusId != 21) && (record.StatusId != 19)) {
                            printInvoice(order_id, order_order_id);
                        } else {
                            Ext.MessageBox.alert('Notification', 'Order is not ready to print.');
                        }
                    }
                },
                {
    ...
    var printInvoice = function () {
        var order_generated_id = arguments[1];
        var order_auto_id = arguments[0];
        //var printOrderPanel = createprintInvoicePanel(order_auto_id, order_generated_id);
        var t = new Date();
        var t_stamp = t.format("YmdHis");
        var src = '?module=order_processing&op=downloadInvoice&order_id=' + order_auto_id + '&order_order_id=' + order_generated_id + "&apikey=" + _SESSION.apikey + "&tstamp=" + t_stamp;
    ...
    var manageOrderStoreb2cso = function () {
        ...
        var store = new Ext.data.GroupingStore({
            ...
            reader: new Ext.data.JsonReader({
                ...
                'dispatch_courier', 'dispatch_consignment', 'delivery_at_date', 'delivery_at_time', 'delivery_notes', 'StatusId', 'br_Name', 'order_HasReturn', 'order_ItemsReturned', 'order_ReturnVerified', 'paymentMode', 'fsto_uid','SONumber','id',
            ]),
            sortInfo: {
                field: 'id',
                direction: "DESC"
