<?php
//User Id for the Particular variable
$order_id = $_REQUEST['order_auto_id'];

global $db;
if ($order_id > 0) {

    $data = $db->getFromDB(" SELECT order_id ,order_order_id,order_group_id, order_total_amount,order_cess,order_delivery_charge,total_afterpacking,order_courier_charge,br_Name,
        order_customer_id, payment_mode,order_method,DATE_FORMAT(bco.created_at,'%d-%m-%Y') AS order_created_on,
 TIME_FORMAT(CAST(bco.created_at AS TIME),'%r') AS ordertime,CASE
    WHEN order_method = 1 THEN 'Drive Delivery'
    WHEN order_method = 2 THEN 'Customer Collect'
    WHEN order_method = 3 THEN 'Courier Delivery'
END AS order_method,
            admin_description as order_status,order_total_gst,DATE_FORMAT(order_confirm_date,'%d-%m-%Y') as order_confirm_date,
            total,order_app_version,order_app_os "
            . " FROM retaline_customer_order bco "
            . " inner join retaline_customer_order_status bcos ON bcos.status_id = bco.status_id INNER JOIN finascop_branch ON br_ID = order_branch_id "
            . " WHERE order_id =' " . $order_id . "'", true);

    $SONumber = $db->getItemFromDB("SELECT SONumber FROM B2CSalesOrder WHERE customer_order_id = {$order_id}");

    $pdts = $db->getMultipleData("SELECT item_product_id,(SELECT stit_SKU 
                    FROM `finascop_stock_itemmaster` WHERE stit_ID=item_product_id) AS product_name,item_order_qty,item_order_qty_scanned,item_sales_price,
                    item_price,item_cgst,item_amount,
                    item_sales_price
                    FROM retaline_customer_order_items
                    WHERE customer_order_id ={$order_id}", true);
    $customerDetails = $db->getFromDB("SELECT cust_customer_name,cust_email,cust_mobile,cust_walletbalance "
            . "FROM retaline_customer WHERE cust_id = {$data['order_customer_id']}", true);
    $deliveryDetails = $db->getFromDB("SELECT order_pin,order_house_no,order_house_name,order_land_mark,order_city,order_latitude,"
            . "order_longitude FROM retaline_customer_order_delivery_address WHERE customer_order_id = {$order_id}", true);
    $paymentDetails = $db->getMultipleData("SELECT fees,currency,mojo_id,DATE_FORMAT(created_at,'%d-%m-%Y') as  created_at,"
            . "amount,instamojo_id,status,redirect_url,payment_status,id "
            . "FROM retaline_paymentgateway_instamojo WHERE order_id = {$order_id} ORDER BY id DESC", true);
    $toDetails = $db->getFromDB("SELECT fsto_id,fstr_id FROM finascop_stock_transfer_order WHERE fsto_ordertype = 1 AND fstr_id = {$order_id}", true);
   //echo "SELECT fsto_id,fstr_id FROM finascop_stock_transfer_order WHERE fsto_ordertype = 1 AND fstr_id = {$order_id}";
    $itemCountDetails = $db->getItemFromDB("SELECT SUM(item_order_qty) FROM retaline_customer_order_items WHERE customer_order_id ={$order_id}");

    if ($toDetails['fsto_id'] > 0) {
        
        //echo "SELECT SUM(fsto_ItemQty) AS fsto_pkdQty,SUM(fsto_pkdQty) AS fsto_pkdQty FROM finascop_stock_transfer_order_details WHERE fsto_id = {$toDetails['fsto_id']}";
    }
}
?>

<html>
    <style>
        .cesstable {
            border: 1px solid #CECECE;
            text-align: left;
        }
        table {
            font-family: arial;
            font-size: 11px;
            border-collapse: collapse;
            border-spacing: 0;
        }
        h4 {
            font-family: arial;
            font-size: 13px;
            font-weight: bold;
            padding: 3px 0;
        }
        .cesstable td {
            border-color: -moz-use-text-color #CECECE #CECECE -moz-use-text-color;
            border-style: none solid solid none;
            border-width: 0 1px 1px 0;
            height: 22px;
            padding: 0 10px 0 12px;
            vertical-align: middle;
        }
    </style>
    <?php
    if (!empty($customerDetails)) {
        ?>
        <h4>Customer  Details</h4>
        <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
            <tbody>



                <tr>
                    <td width="30%">
                        Name
                    </td>
                    <td>
                        <b> <?php echo $customerDetails['cust_customer_name']; ?> </b>
                    </td>
                </tr>

                <tr>
                    <td>
                        Email
                    </td>
                    <td>
                        <b> <?php echo $customerDetails['cust_email']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Mobile
                    </td>
                    <td>
                        <b> <?php echo $customerDetails['cust_mobile']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Delivery Address
                    </td>
                    <td>
                        <b> <?php echo $deliveryDetails['order_house_name']; ?> </br>
                            <?php echo $deliveryDetails['order_land_mark'] . '  ' . $deliveryDetails['order_city'] . ' - ' . $deliveryDetails['order_pin']; ?>
                        </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Latitude-Longitude
                    </td>
                    <td>
                        <b> <?php echo $deliveryDetails['order_latitude'] . '-' . $deliveryDetails['order_longitude']; ?> </b>
                    </td>
                </tr>
            </tbody>
        </table>
    <?php } ?>
    <?php
    if (!empty($data)) {
        //echo '<pre>';print_r($data);
        if ($data['total_afterpacking'] > 0) {
            $t_amout = $data['total_afterpacking'];
        } else {
        $t_amout = $data['total'];
        }

        // $t_amout =  $data['order_total_amount'] + $data['order_delivery_charge'];
        ?>
        <h4>Order  Details</h4>
        <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
            <tbody>
                <tr>
                    <td>
                        Sales Order Number
                    </td>
                    <td>
                        <b> <?php echo $SONumber; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Order Number
                    </td>
                    <td>
                        <b> <?php echo $data['order_order_id']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Order Group
                    </td>
                    <td>
                        <b> <?php echo $data['order_group_id']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Store
                    </td>
                    <td>
                        <b> <?php echo $data['br_Name']; ?> </b>
                    </td>
                </tr><tr>
                    <td>
                        Date
                    </td>
                    <td>
                        <b> <?php echo $data['order_created_on']; ?> </b>
                    </td>
                </tr><tr>
                    <td>
                        Time
                    </td>
                    <td>
                        <b> <?php echo $data['ordertime']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Store
                    </td>
                    <td>
                        <b> <?php echo $data['br_Name']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Delivery Mode
                    </td>
                    <td>
                        <b> <?php echo $data['order_method']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Order Quantity
                    </td>
                    <td>
                        <b> <?php echo $itemCountDetails; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td width="30%">
                        Amount
                    </td>
                    <td>
                        <b> <?php echo $t_amout; ?> </b>
                    </td>
                </tr>

                <tr>
                    <td>
                        Created On
                    </td>
                    <td>
                        <b> <?php echo $data['order_confirm_date']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Order Status
                    </td>
                    <td>
                        <b> <?php echo $data['order_status']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        Payment Mode
                    </td>
                    <td>
                        <?php
                        switch ($data['payment_mode']) {
                            case 1:
                                $payMod = 'Pay On Delivery';
                                break;
                            case 2:
                                $payMod = 'Online Payment';
                                break;
                            case 3:
                                $payMod = 'Wallet';
                                break;
                            case 4:
                                $payMod = 'COD with Wallet';
                                break;
                            case 5:
                                $payMod = 'Online with Wallet';
                                break;
                            case 6:
                                $payMod = 'Online on Delivery';
                                break;
                            case 7:
                                $payMod = 'Cash on Delivery';
                                break;
                        }
                        ?>
                        <b> <?php echo $payMod; ?> </b>
                    </td>
                </tr>
    <!--                <tr>
                    <td>
                        App Version
                    </td>
                    <td>
                        <b> <?php echo $data['order_app_version']; ?> </b>
                    </td>
                </tr>
                <tr>
                    <td>
                        App OS
                    </td>
                    <td>
                        <b> <?php echo $data['order_app_os']; ?> </b>
                    </td>
                </tr>-->
    <!--                <tr>
                    <td>
                        Delivery Mode
                    </td>
                    <td>
                        <?php
                        switch ($data['order_method']) {
                            case 1:
                                $order_method = 'Drive Delivery';
                                break;
                            case 2:
                                $order_method = 'Customer Collect';
                                break;
                            case 3:
                                $order_method = 'Courier Delivery';
                                break;
                        }
                        ?>
                        <b> <?php echo $order_method; ?> </b>
                    </td>
                </tr>-->
            </tbody>
        </table>
<?php } else { ?>
        sorry there is no available data to display
    <?php } ?>

    <?php if (($_SESSION['admin']->IS_RETALINE_LITE != 1) && ($toDetails['fsto_id'] > 0)) { ?>
        <h4>Barcode Details</h4>
        <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
            <tbody>
                <tr>
                    <td width="30%"><strong>Barcode</strong></td>
                    <td width="20%"><strong>Item Name</strong></td>
                </tr>
    <?php
    $barcodes = $db->getMultipleData("SELECT stiid_barcode,stiid_id FROM finascop_stock_transfer_order_details_barcodes WHERE fsto_id = {$toDetails['fsto_id']}", true);
    foreach ($barcodes as $barcod) {
        $stiid_itemmastername = $db->getItemFromDB("SELECT stiid_itemmastername FROM finascop_stock_item_inventorydetails WHERE stiid_id = {$barcod['stiid_id']}");
        ?>
                    <tr><td width = "25%"><?php echo $barcod['stiid_barcode']; ?></td>
                        <td width="25%"><?php echo $stiid_itemmastername; ?></td>
                    </tr>
    <?php }
    ?>

            </tbody>
        </table>
<?php } ?>
<?php if (!empty($pdts)) { ?>
        <h4>Product Details</h4>
        <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
            <tbody><tr>
                    <td width="60%"><strong>Name</strong></td>
                    <td width="20%"><strong>Quantity</strong></td>
    <!--                    <td width="25%"><strong>Price without tax</strong></td>-->
                    <td width="20%"><strong>Total</strong></td>
                </tr>

    <?php
    $sum = 0;
    $price_sum = 0;
    $with_tax = 0;
    foreach ($pdts as $key2 => $value2) {
        $packageType = $db->getFromDB("SELECT cs_package_type_name,ds_package_type_name,least_package_type_name FROM finascop_stock_itemmaster WHERE stit_ID = {$value2['item_product_id']}", true);
        $sum+= $value2['item_order_qty'];
        $price_sum+=$value2['item_amount'];
        $with_tax+=$value2['item_price'];
                    if ($value2['item_order_qty_scanned'] > 0) {
                        $item_price = $value2['item_order_qty_scanned'] * $value2['item_sales_price'];
                        $item_price = round($item_price, 2);
                    } else {
                        $item_price = $value2['item_price'];
                    }
                    $item_order_qty = floatval($value2['item_order_qty']);
        echo "<tr>"
        . "<td>{$value2['product_name']}</td> "
                    . "<td style='text-align: right;padding-right: 10px;'>{$item_order_qty}</td>"
//        . "<td>{$value2['item_amount']}</td>"
                    . " <td style='text-align: right;padding-right: 10px;'>{$item_price}</td>"
        . "</tr> ";
    }
                $handlingCharge = number_format(floatval($data['order_delivery_charge'] + $data['order_courier_charge']),2);
    ?>
                <tr>
                    <td width="25%"><strong>Handling Charge</strong></td>
                    <td width="25%"><strong><?php echo '-' ?></strong></td>
    <!--                    <td width="25%"><strong><?php echo '-' ?></strong></td>-->
                    <td width="25%" style='text-align: right;padding-right: 10px;'><strong><?php echo $handlingCharge; ?></strong></td>
                </tr>
                <tr>
                    <td width="25%"><strong>CESS</strong></td>
                    <td width="25%"><strong><?php echo '-' ?></strong></td>
    <!--                    <td width="25%"><strong><?php echo '-' ?></strong></td>-->
                    <td width="25%" style='text-align: right;padding-right: 10px;'><strong><?php echo $data['order_cess']; ?></strong></td>
                </tr>
                <tr>
                    <td width="25%"><strong>Total</strong></td>
                    <td width="25%" style='text-align: right;padding-right: 10px;'><strong><?php echo $sum ?></strong></td>
    <!--                    <td width="25%"><strong><?php echo $price_sum ?></strong></td>-->
                    <td width="25%" style="text-align: right;padding-right: 10px;"><strong><?php echo $t_amout ?></strong></td>
                    <!--<td width="25%"><strong><?php echo $with_tax + $data['order_delivery_charge'] ?></strong></td>-->
                </tr>
            </tbody></table>
           <!-- <h4 style="float:right">Total Product Quantity : <?php //echo $sum               ?> </h4> -->

<?php } ?>
<?php if (!empty($paymentDetails)) { ?>
        <h4>Payment Details</h4>
        <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
            <tbody><tr>
                    <td width="30%"><strong>ID</strong></td>
                    <td width="15%"><strong>Amount</strong></td>
                    <td width="15%"><strong>Fees</strong></td>
                    <td width="15%"><strong>Payment status</strong></td>
                    <td width="10%"><strong>Status</strong></td>
                    <td width="15%"><strong>Create On</strong></td>
                </tr>

    <?php
    $sum = 0;
    $price_sum = 0;
    $with_tax = 0;
    for ($j = 0; $j < count($paymentDetails); $j++) {
        if ($paymentDetails[$j]['payment_status'] == 0) {
            $ps = 'Pending';
        } else if ($paymentDetails[$j]['payment_status'] == 1) {
            $ps = 'Completed';
        } else if ($paymentDetails[$j]['payment_status'] == 2) {
            $ps = 'Failed';
        }
        echo "<tr>"
        . "<td>{$paymentDetails[$j]['mojo_id']}</td> "
        . "<td>{$paymentDetails[$j]['amount']}</td>"
        . "<td>{$paymentDetails[$j]['fees']}</td>"
        . " <td>{$ps}</td>"
        . " <td>{$paymentDetails[$j]['status']}</td>"
        . "<td>{$paymentDetails[$j]['created_at']}</td>"
        . "</tr> ";
    }
    ?>

            </tbody></table>

<?php } ?>




</html>
