...
switch ($toDetails['fsto_ordertype']) {
    ...
    case 1:
        ...
        $parentOrder = $db->getFromDB("SELECT order_order_id AS paOrderNumber,order_confirm_date AS paOrderDate,order_slot_id,order_slot_date,payment_mode FROM retaline_customer_order WHERE order_id = {$toDetails['fstr_id']}", true);
        break;
    case 2:
        $fsto_ordertype = 'B2B';
        ...
        if ($toDetailItems[0]['fsto_ItemId'] > 0) {
            ...
            <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
                <tbody>
                    ...
                    <tr><td><b>Item Name</b></td><td><b>Ordered Items</b></td><td><b>Picked Items</b></td><td><b>Required Qty</b></td><td><b>Packed Qty</b></td></tr>
                    <?php
                    for ($i = 0; $i < count($toDetailItems); $i++) {
                        $packageType = $db->getFromDB("SELECT cs_package_type_name,ds_package_type_name,stit_ParentItemId,least_package_type_name,stit_ConvertCalcRate FROM finascop_stock_itemmaster WHERE stit_ID = {$toDetailItems[$i]['fsto_ItemId']}", true);

                        if ($packageType['stit_ParentItemId'] > 0) {
                            $orderQty =$toDetailItems[$i]['fsto_ItemQty']* $packageType['stit_ConvertCalcRate'];
                            $packQty = $toDetailItems[$i]['fsto_stockValue'];
                        ...
                        <tr><td><?php
                                ...
                                echo $fsto_ItemName;
                                ?></td>
                            <td style='text-align: right;padding-right: 10px;'><?php echo $toDetailItems[$i]['fsto_ItemQty']; ?></td>
                            <td style='text-align: right;padding-right: 10px;'><?php echo $toDetailItems[$i]['fsto_pkdQty']; ?></td>
                            <td style='text-align: right;padding-right: 10px;'><?php echo $orderQty . ' ' . $packTyp; ?></td>
                            <td style='text-align: right;padding-right: 10px;'><?php echo $packQty . ' ' . $packTyp; ?></td>
                        </tr>
                    <?php } ?> 
