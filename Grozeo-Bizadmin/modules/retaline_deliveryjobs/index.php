...
switch ($op) {
    ...
    case 'saveQugeo':
        ...
        $packingOrderIds = array_column($geoData, 'quor_id');
        ...
        if ($type == 'PICKUP') {
            $orderid = $scheduleorder->scheduleABookingSchJobs($bk_id, $orderdetails, true, $v_id, true, $packingOrderIds, 1, 1, 'Nor');
            //$orderid = $scheduleorder->scheduleABooking($bk_id, $orderdetails, true, $v_id, true);
        } else {
            $orderid = $scheduleorder->scheduleADeliverySchJobs($bk_id, $orderdetails, true, $v_id, true, false, $handling_br_id, $packingOrderIds, 1, 1, 'Nor');
        }
    ...
    case 'saveCustomerPickup':
        ...
        if (!empty($_POST['bankTransactionNo']))
            $data['bankTransactionNo'] = $_POST['bankTransactionNo'];
        if (!empty($_POST['deliveredAmtType']))
            $data['deliveredAmtType'] = $_POST['deliveredAmtType'];
    ...
    case 'saveManualDelivery':
        ...
        $qgeoStatus['quor_UpdateOn'] = date("Y-m-d H:i:s");
        $qgeoStatus['quor_Status'] = 9;
        $quor_DeliveryConfTime = date('Y-m-d H:i:s', strtotime($quor_DeliveryConfTime));
        ...
        if ($qugeoDetails['quor_AmountCollectible'] > 0) {
    ...
    case 'setDrive':
        ...
        $db->query('begin');
        $qgeoStatus['quor_UpdateOn'] = date("Y-m-d H:i:s");
        $qgeoStatus['quor_Type'] = 1;
        //$qgeoStatus['quor_Status'] = 22;
        $status = $db->perform('qugeo_order', $qgeoStatus, 'update', "quor_id = {$quor_id}");
        $status = $db->query('commit');
        if ($status == 1) {
            echo "{success:true,valid:true,message:'Order dispatched'}";
        } else {
