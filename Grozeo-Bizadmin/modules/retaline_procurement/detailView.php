...
switch ($toDetails['fsto_ordertype']) {
    ...
    case 1:
        ...
        $b2corder = $db->getFromDB("SELECT order_id,total,total_afterpacking FROM retaline_customer_order WHERE order_id = {$toDetails['fstr_id']}", true);
        break;
    case 2:
        $fsto_ordertype = 'B2B';
...
//        echo count($countDetails);
        ...
        <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
            <tbody>
                ...
                <?php if ($b2corder['order_id'] > 0) {
                    if($b2corder['total_afterpacking'] > 0){
                        $total = $b2corder['total_afterpacking'];
                    }else{
                        $total = $b2corder['total'];
                    }
                    ?>
                <tr>
                    <td>
        ...
        if ($toDetailItems[0]['fsto_ItemId'] > 0) {
            ...
            <table width="100%" cellspacing="2" cellpadding="2" border="0" class="cesstable">
                <tbody>
                    ...
                    <tr><td><b>Item Name</b></td><td><b>Required Qty</b></td><td><b>Packed Qty</b></td></tr>
                    <?php
                    for ($i = 0; $i < count($toDetailItems); $i++) {
                        ...
                        if ($packageType['stit_ParentItemId'] > 0) {
                            ...
                            $orderQty = $toDetailItems[$i]['fsto_ItemQty'];
                            $packQty = $toDetailItems[$i]['fsto_pkdQty'];
                            if ($toDetails['fsto_ordertype'] == 1) {
                                $packTyp = '';
                            } else {
                                $packTyp = $packageType['least_package_type_name'];
                        }
