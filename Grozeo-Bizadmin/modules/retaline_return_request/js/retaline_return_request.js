                    ...
                    items: [
                        {
                            ...
                            getClass: function (v, meta, rec) {
                                ...
                                console.log('item_return_qty', rec.get('item_return_qty'));
                                console.log('item_return_qty_requested', rec.get('item_order_qty'));
                                console.log('item_order_qty', rec.get('item_order_qty'));
                                console.log('item_order_qty', rec.get('item_order_qty'));
                            ...
                            handler: function (grid, rowIndex, colIndex) {
                                ...
                                upReturnQty(order_id, itemId, item_order_qty, record, itemName,ordertype);
...
//                                            Ext.MessageBox.alert("Notification", tmp.msg);
    ...
    var _orderItemStore = function (order_id, orderType) {
    ...
    var upReturnQty = function (order_id, itemId, item_order_qty, record, itemName,ordertype) {
    ...
    var returnOrderDetails = function () {
        return new Ext.Panel({
            ...
            tpl: new Ext.XTemplate('<div class="details-outer"  colspan=3>',
                    ...
                    '<tr><th width="20%">Order No.</th><td> {order_order_id} </td><th width="20%">Date</th><td> {orderdate}</td></tr>',
                    '<tr><th width="20%">Total</th><td> {total}</td><th width="20%">Delivered On</th><td> {deliveryDate}</td></tr>',
                    '<tr><th width="20%">Customer </th><td> {cust_customer_name} </td><th width="20%">Contact No </th><td> {cust_mobile} </td></tr>',
                    '<tr></tr>',
                    '</table>',
                    '</div>'
                    )
...
//                        }
    ...
    var customerReturnRequestTopPanel = function (id) {
        ...
        var panel = new Ext.Panel({
            ...
            items: [manageReturnRequestGrid(),
                new Ext.Panel({
                    ...
                    width: winsize.width * 0.45,
                    items: [{
                            id: 'custorder_returnreq_details_view_panel',
                            hidden: true,
                            html: '<iframe id="iframe_returnreq_dtls" name="iframe_returnreq_dtls"   frameborder="0" style="overflow:hidden;height:100%;width:100%" height="100%" width="100%  src="' + src + '"; ></iframe>',
                        }]
                })
            ]
        });
    ...
    var manageReturnRequestGrid = function () {
        var BranchStore = new Ext.data.JsonStore({
            fields: ['br_ID', 'br_Name'],
            url: brmodURL + '&op=getBranchName',
            autoLoad: true,
        ...
        hfilters.remote = true;
        hfilters.autoReload = true;
        var order_store = manageReturnRequestStore();
        var grid = new Ext.grid.GridPanel({
            store: order_store,
            region: 'center',
            frame: true,
            ...
            plugins: [new Ext.ux.grid.GroupSummary(), hfilters],
            id: 'manage_customer_returnreq_grid',
            columns: [new Ext.grid.RowNumberer(),
                {
                    header: 'Order ID',
                    sortable: true,
            ...
            sm: new Ext.grid.RowSelectionModel({
                singleSelect: true,
                listeners: {
                    selectionchange: returnRequestgridSelectionChanged
                }
            }), tbar: [{
                    html: '&nbsp;BRANCH : &nbsp;',
                },
                {
                    xtype: 'textfield',
                    id: 'textboxCustomerReturnReq',
                    name: 'textboxCustomerReturnReq',
                    anchor: '98%',
                    tabIndex: 1,
                    maxLength: 20,
                    setReadOnly: true,
                ...
                {
                    xtype: 'combo',
                    id: 'comboxCustomerReturnReq',
                    name: 'comboxCustomerReturnReq',
                    mode: 'local',
                    typeAhead: true,
                    forceSelection: true,
                    fieldLabel: 'Branch Name',
                    ...
                    displayField: 'br_Name',
                    valueField: 'br_ID',
                    hiddenName: 'comboxCustomerReturnReq',
                    listeners: {
                        select: function () {
                            var branchName = Ext.getCmp('comboxCustomerReturnReq').getRawValue();
                            Ext.getCmp('manage_customer_returnreq_grid').getStore().load({
                                params: {
                                    br_Name: branchName
                                }
                            });
            ...
            listeners: {
                ...
                afterrender: function () {
                    if (_SESSION.current_branch_iscpd == 1) {
                        Ext.getCmp("comboxCustomerReturnReq").show();
                        Ext.getCmp("textboxCustomerReturnReq").hide();
                    } else {
                        Ext.getCmp("textboxCustomerReturnReq").show();
                        Ext.getCmp("textboxCustomerReturnReq").setValue(_SESSION.current_branch);
                        Ext.getCmp("textboxCustomerReturnReq").hide();
                    }
                    if (Ext.getCmp('textboxCustomerReturnReq').getValue() != '')
                    {
                        order_store.baseParams = {
                            current_branch_id: _SESSION.finascop_current_branch_id
                        }
    ...
    var returnRequestOrderMenu = function (e) {
        var returnOrderMenu = new Ext.menu.Menu({
            items: [{
                    text: "Create Return Order",
                    handler: function () {
                    var order_id = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.order_id;
                    var order_customer_id = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.order_customer_id;
                    var updated_at = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.updated_at;
                    var status_id = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.status;
                    var t = new Date();
                    var t_stamp = t.format("YmdHis");
                    Ext.Ajax.request({
                        url: modURL + '&op=generateUniqueId',
                        ...
                        success: function (res) {
                            ...
                            var uid = tmp.uid;
                            Application.ReturnRequest.Cache.uuid = uid;
                            Application.ReturnRequest.returnOrder(order_id, order_customer_id, updated_at, status_id, "B2C");
                        }
                    });
                    }
                }]
    ...
    var returnRequestgridSelectionChanged = function (sm) {

        if (!Ext.isEmpty(Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections())) {
            var ID = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.order_id;
            var rtrqo_sourceType = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.rtrqo_sourceType;
            Application.ReturnRequest.Cache.orderAutoId = ID;
            Application.ReturnRequest.Cache.rtrqo_sourceType = rtrqo_sourceType;
            Application.ReturnRequest.RRViewMode(ID, rtrqo_sourceType);
        }
    };
    var manageReturnRequestStore = function () {

        var store = new Ext.data.GroupingStore({
            proxy: new Ext.data.HttpProxy({
                url: modURL + '&op=getReturnRequestOrders',
                method: 'post'
            }),
            reader: new Ext.data.JsonReader({
                totalProperty: 'totalCount',
                ...
                'dispatch_courier', 'reason', 'cancelled_by_type_name', 'cancelled_by_type', 'cancelled_by_name', 'cancelled_by_id', , 'dispatch_consignment', 'delivery_at_date', 'delivery_at_time', 'delivery_notes',
                'status', 'br_Name', 'order_HasReturn', 'order_ItemsReturned', 'order_ReturnVerified', 'order_itemReturnRequestCount', 'order_qty', 'cust_mobile', 'rtrqo_sourceType','updated_at'
            ]),
            sortInfo: {
                field: 'order_id',
                direction: "DESC"
            ...
            listeners: {
                ...
                load: function (store, e) {
                    Ext.getCmp('manage_customer_returnreq_grid').getView().refresh();
                    Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().selectRow(0);
                    if (!Ext.isEmpty(Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections())) {
                        var ID = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.order_id;
                        var rtrqo_sourceType = Ext.getCmp('manage_customer_returnreq_grid').getSelectionModel().getSelections()[0].data.rtrqo_sourceType;
                        Application.ReturnRequest.Cache.orderAutoId = ID;
                        Application.ReturnRequest.Cache.rtrqo_sourceType = rtrqo_sourceType;
                        Application.ReturnRequest.RRViewMode(Application.ReturnRequest.Cache.orderAutoId, Application.ReturnRequest.Cache.rtrqo_sourceType);
                    }
                }
            }
        });
...
//                            icon: IMAGE_BASE_PATH + '/default/icons/disk.png',
        ...
        addBannedStock: function () {
        ...
        },customerReturnRequest: function () {

            var panelId = 'customer_returnreq_processing_panel';
            var CustomerReturnRequestPanel = Ext.getCmp(panelId);
            if (Ext.isEmpty(CustomerReturnRequestPanel)) {
                CustomerReturnRequestPanel = customerReturnRequestTopPanel(panelId);
                Application.UI.addTab(CustomerReturnRequestPanel);
                CustomerReturnRequestPanel.doLayout();
            } else {
                Application.UI.addTab(CustomerReturnRequestPanel);
            }
            ...
            var order_auto_id = arguments[0];
            var rtrqo_sourceType = arguments[1];
            Ext.getCmp('custorder_returnreq_details_view_panel').show();

            Ext.get('iframe_returnreq_dtls').dom.src = modURL + '&op=return_request_details&order_auto_id=' + order_auto_id + '&rtrqo_sourceType=' + rtrqo_sourceType + "&apikey=" + _SESSION.apikey + "&tstamp=" + t_stamp;
