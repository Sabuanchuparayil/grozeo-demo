...
$rtrqo_sourceType = $_REQUEST['rtrqo_sourceType'];

global $db;
if ($order_id > 0) {

    if ($rtrqo_sourceType == 3) {
        $data = $db->getFromDB(" SELECT bbso_id as order_id ,bbso_SONumber as order_order_id, bbso_SOValue as order_total_amount,'' as order_delivery_charge,b2b_Customer_ID as order_customer_id,'' as payment_mode,
            status as order_status,(bbso_CGSTVal + bbso_SGSTVal) as order_total_gst,DATE_FORMAT(bbso_createdon,'%d-%m-%Y') as order_confirm_date,bbso_SOValue as total "
        ...
        $pdts = $db->getMultipleData("SELECT b2bso_itemid as item_product_id,(SELECT stit_SKU 
                    FROM `finascop_stock_itemmaster` WHERE stit_ID=item_product_id) AS product_name,b2bso_itemqty as item_order_qty,
                    b2bso_itemrate as item_price,b2bso_cgst_value as item_cgst,
                    b2bso_netamount as item_sales_price
                    FROM `retaline_B2B_SalesOrderDetails`
                    WHERE bbso_id ={$order_id}", true);
                    $customerDetails = $db->getFromDB("SELECT b2b_Customer_Name as cust_customer_name,b2b_Customer_Email as cust_email,b2b_Customer_Mobile as cust_mobile,'' as cust_walletbalance FROM retaline_B2Bcustomer WHERE b2b_Customer_ID = {$data['order_customer_id']}", true);
...
<?php } ?>
<?php if (!empty($paymentDetails) && ($rtrqo_sourceType != 3)) { ?>
