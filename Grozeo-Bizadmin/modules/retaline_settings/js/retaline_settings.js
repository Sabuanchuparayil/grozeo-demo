Application.RetalineSettings = (function () {
  ...
  var gridSelectionChangedcourier = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("gridpanelMasterDataviewCourierdata")
          .getSelectionModel()
          .getSelections()
      )
    ) {
      var ID = Ext.getCmp("gridpanelMasterDataviewCourierdata")
        .getSelectionModel()
        .getSelections()[0].data.mst_courier_id;
  ...
  var gridSelectionChangedfaq = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("faqMasterGrid").getSelectionModel().getSelections()
      )
    ) {
      var ID = Ext.getCmp("faqMasterGrid")
        .getSelectionModel()
        .getSelections()[0].data.faq_id;
  ...
  var gridSelectionChanged = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("pageMasterGrid").getSelectionModel().getSelections()
      )
    ) {
      var ID = Ext.getCmp("pageMasterGrid")
        .getSelectionModel()
        .getSelections()[0].data.page_id;
  ...
  var gridSelectionFeedbackChanged = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("feedBackGridPanel").getSelectionModel().getSelections()
      )
    ) {
      var ID = Ext.getCmp("feedBackGridPanel").getSelectionModel().getSelections()[0].data.fb_id;
      var fbStatus = Ext.getCmp("feedBackGridPanel").getSelectionModel().getSelections()[0].data.status;        
  ...
  var gridSelectionappNotificationChanged = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("appNotificationGridPanel")
          .getSelectionModel()
          .getSelections()
      )
    ) {
      var ID = Ext.getCmp("appNotificationGridPanel")
        .getSelectionModel()
        .getSelections()[0].data.notification_id;
  ...
  var gridSelectionpaymentTermsChanged = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("paymentTermsGridPanel").getSelectionModel().getSelections()
      )
    ) {
      var ID = Ext.getCmp("paymentTermsGridPanel")
        .getSelectionModel()
        .getSelections()[0].data.ptc_id;
  ...
  var gridSelectionChangedstoregroups = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("gridpanelMasterDataviewStoreGroupsdata")
          .getSelectionModel()
          .getSelections()
      )
    ) {
      var ID = Ext.getCmp("gridpanelMasterDataviewStoreGroupsdata")
        .getSelectionModel()
  ...
  var saveFaq = function () {
    ...
    if (
      !Ext.isEmpty(
        Ext.getCmp("faq_title").getValue() &&
          Ext.getCmp("faq_description").getValue() &&
          Ext.getCmp("faq_status").getValue()
      )
    ) {
      Ext.Ajax.request({
      ...
      items: [
        {
          ...
          listeners: {
            select: function () {
              var page_id = Ext.getCmp("page_id").getValue();
              if (page_id == 0 || Ext.isEmpty(page_id)) {
                var pageTitle = Ext.getCmp("page_type").getRawValue();
                Ext.getCmp("page_name").setValue(pageTitle);
              }
            },
          },
        },
  ...
  var savePage = function () {
    ...
    if (
      !Ext.isEmpty(
        Ext.getCmp("page_name").getValue() &&
          Ext.getCmp("page_content").getValue() &&
          Ext.getCmp("page_status").getValue()
      )
    ) {
      Ext.Ajax.request({
        ...
        method: "POST",
        params: {
          page_type: Ext.getCmp("page_type").getValue(),
          ...
          fbar: [
            {
              ...
              handler: function () {
                if (
                  !Ext.isEmpty(
                    Ext.getCmp("feedBackGridPanel")
                      .getSelectionModel()
                      .getSelections()
                  )
                ) {
                  var ID = Ext.getCmp("feedBackGridPanel")
                    .getSelectionModel()
                    .getSelections()[0].data.fb_id;
                  Application.RetalineSettings.ViewFeedbackMode(ID);
                }
              },
            },
  ...
  var feedbackGridstore = function () {
    var _feedbackList = new Ext.data.GroupingStore({
      ...
      reader: new Ext.data.JsonReader(
        ...
        ["fb_id", "fb_mobile", "fb_email", "fb_comments",'storegroup_id','fb_createdOn','storegroupName','status','statusName']
      ),
      sortInfo: {
        field: "fb_id",
        direction: "ASC",
  ...
  var feedbackActionMenu = function (e) {
    var feedbackActionMenu = new Ext.menu.Menu({
      items: [          
        {
          text: "Attended",
          handler: function () {
            var fb_id = Ext.getCmp("feedBackGridPanel").getSelectionModel().getSelections()[0].data.fb_id;
              Ext.MessageBox.confirm('Confirm', 'Feeedback status moved to Attended?', function (btn, text) {
                if (btn == 'yes') {
                  Ext.Ajax.request({
                    waitMsg: 'Processing',
                    method: 'POST',
                    url: modURL + '&op=feedbackActions',
                    params: {
                      fb_id: fb_id,
                      action:2
                    },
                    success: function (response) {
                        var tmp = Ext.util.JSON.decode(response.responseText);
                        if (tmp.success === true) {          
                            Ext.getCmp('feedBackGridPanel').getStore().reload();          
                        }
                    },
                    failure: function (response) {
                        var tmp = Ext.util.JSON.decode(response.responseText);
          ...
          handler: function () {
            var fb_id = Ext.getCmp("feedBackGridPanel").getSelectionModel().getSelections()[0].data.fb_id;
              Ext.MessageBox.confirm('Confirm', 'Do you want to remove the enquiry?', function (btn, text) {
                if (btn == 'yes') {
                  Ext.Ajax.request({
                    waitMsg: 'Processing',
                    method: 'POST',
    ...
    feedbackActionMenu.showAt(e.getXY());
  };
    ...
    var _notificationPanel = new Ext.Panel({
      ...
      items: [
        ...
        new Ext.Panel({
          ...
          height: winsize.height * 0.6,
          items: [notificationForm(), notificationDetailsView()],
          buttonAlign: "right",
          fbar: [
            {
              text: "Cancel",
  ...
  var notificationGridstore = function () {
    var _notificationList = new Ext.data.GroupingStore({
      ...
      reader: new Ext.data.JsonReader(
        ...
        ["notification_id", "notification_content", "notification_status","notification_remarks"]
      ),
      sortInfo: {
      ...
      listeners: {
        afterrender: function () {
          if (Ext.isEmpty(Ext.getCmp("notification_id").getValue())) {
            var recordSelected = Ext.getCmp("notification_status")
              .getStore()
              .getAt(0);
            Ext.getCmp("notification_status").setValue(
  ...
  var notificationDetailsView = function () {
    return new Ext.Panel({
      ...
      tpl: new Ext.XTemplate(
        '<div class="details-outer">',
        '<table border="0" width="100%" class="details_view_table">',
        '<tr><th width="40%">Title </th><td>  {notification_remarks} </td></tr>',
        '<tr><th width="40%">Content </th><td>  {notification_content} </td></tr>',
  ...
  var notificationGrid = function () {
    ...
    var _notificationgridPanel = new Ext.grid.GridPanel({
      ...
      columns: [
        ...
        {
          header: "Title",
          dataIndex: "notification_remarks",
          sortable: true,
          tooltip: "Title",
        },
        {
    ...
    var _adPanel = new Ext.Panel({
      ...
      items: [
        ...
        new Ext.Panel({
          ...
          height: winsize.height * 0.6,
          items: [paymentTermsForm(), paymentTermsDetailsView()],
          buttonAlign: "right",
          fbar: [
            {
              text: "Cancel",
              ...
              handler: function () {
                if (
                  !Ext.isEmpty(
                    Ext.getCmp("paymentTermsGridPanel")
                      .getSelectionModel()
                      .getSelections()
                  )
                ) {
  ...
  var savePaymentTerms = function () {
    ...
    if (
      !Ext.isEmpty(
        Ext.getCmp("ptc_name").getValue() &&
          Ext.getCmp("ptc_days").getValue() &&
          Ext.getCmp("ptc_status").getValue()
      )
    ) {
      Ext.Ajax.request({
      ...
      listeners: {
        afterrender: function () {
          if (Ext.isEmpty(Ext.getCmp("mst_courier_id").getValue())) {
            var recordSelected = Ext.getCmp("comboMasterCourierStatus")
              .getStore()
              .getAt(0);
            Ext.getCmp("comboMasterCourierStatus").setValue(
              recordSelected.get("id")
            );
          }
        },
      ...
      tbar: [
        {
          ...
          handler: function () {
            ...
            Ext.getCmp("panelMasterCourierParent").setTitle(
  ...
  var saveCourier = function () {
    if (
      !Ext.isEmpty(Ext.getCmp("mst_courier_name").getValue()) &&
        ...
        success: function (response) {
          ...
          if (tmp.success === true && tmp.valid === true) {
            ...
            if (Application.RetalineSettings.CourierAddEdit == "Add") {
              ...
              Ext.getCmp(
                "gridpanelMasterDataviewCourierdata"
    ...
    var _mpanelforCourier = new Ext.Panel({
      ...
      items: [
        ...
        new Ext.Panel({
          ...
          height: winsize.height * 0.6,
          items: [CourierForm(), CourierMasterDetailsView()],
          buttonAlign: "right",
          fbar: [
            /*<?php if (user_access("retaline_settings", "saveCourier")) { ?> */ {
              text: "Edit",
    ...
    var _storegroupsFormPanel = new Ext.form.FormPanel({
      ...
      items: [
        ...
        },
        {
          xtype: "textfield",
          fieldLabel: "Mobile",
          id: "store_group_mobile",
          name: "n[store_group_mobile]",
          anchor: "98%",
          //allowBlank: false,
          tabIndex: 33,
          maxValue: 100,
          ...
          listeners: {
            select: function () {
              var value = Ext.getCmp(
                "store_group_primary_businessType"
              ).getValue();
            ...
            select: function () {
              ...
              if (Ext.isEmpty(primaryBt)) {
                Ext.MessageBox.alert(
                  "Notification",
                  "Choose Primary Businness Before choosing Additional Business."
                );
          ...
          items: [
            {
              ...
              items: [
                {
                  xtype: "checkbox",
                  id: "store_group_outofStock",
                  name: "n[store_group_outofStock]",
                  boxLabel: "Show Out of Stock",
                  allowBlank: true,
                  inputValue: 1,
                  listeners: {
            ...
            },
            {
              ...
              items: [
                {
                  xtype: "checkbox",
                  id: "store_group_grosmartMerchant",
                  name: "store_group_grosmartMerchant",
                  boxLabel: "GroSmart Merchant",
                  allowBlank: true,
                  inputValue: 1,
                  listeners: {
                    check: function (checkbox, checked) {
                      if (checked == true) {
                      Ext.getCmp("exclusiveMerchant").enable();
                      Ext.getCmp("reservedMerchant").enable();
                    }else{
                      Ext.getCmp("exclusiveMerchant").disable();
                      Ext.getCmp("exclusiveMerchant").setValue('');
                      Ext.getCmp("exclusiveMerchantIds").setValue('');
                      Ext.getCmp("reservedMerchant").disable();
                      Ext.getCmp("reservedMerchant").setValue('');
                      Ext.getCmp("reservedMerchantIds").setValue('');
                    }
                    },
                  },
                },
        ...
        {
          xtype: "textarea",
          fieldLabel: 'Merchants with Exclusive B2B listing rights<a href="#" onclick="Application.RetalineSettings.mapMerchants(1)" style="margin-left:320px; text-decoration: underline; color: black;">Manage</a>',
          id: "exclusiveMerchant",
          name: "n[exclusiveMerchant]",
          anchor: "98%",
          tabIndex: 33,
        ...
        {
          xtype: "textfield",
          id: "exclusiveMerchantIds",
          name: "n[exclusiveMerchantIds]",
          hidden: true,
        },
        {
          xtype: "textarea",
          fieldLabel: 'Merchants with Reserved B2B listing price<a href="#" onclick="Application.RetalineSettings.mapMerchants(2)" style="margin-left:320px; text-decoration: underline; color: black;">Manage</a>',
          id: "reservedMerchant",
          name: "n[reservedMerchant]",
          anchor: "98%",          
        },
        {
          xtype: "textfield",
          id: "reservedMerchantIds",
          name: "n[reservedMerchantIds]",
          hidden: true,
        },
        mkCombo({
          type: STATUS_COMBO_DATA,
      ...
      listeners: {
        afterrender: function () {
          if (Ext.isEmpty(Ext.getCmp("store_group_id").getValue())) {
            var recordSelected = Ext.getCmp("comboMasterStoreGroupsStatus")
              .getStore()
              .getAt(0);
            Ext.getCmp("comboMasterStoreGroupsStatus").setValue(
              recordSelected.get("id")
            );
          }
        },
  ...
  var StoreGroupMasterDetailsView = function () {
    return new Ext.Panel({
      ...
      tpl: new Ext.XTemplate(
        ...
        '<tr><th width="40%">Mobile </th><td>  {contactNumber} </td></tr>',
        ...
        '<tr><th width="40%">Show Out of Stock </th><td>  {store_group_outofStock} </td></tr>',
        '<tr><th width="40%">GroSmart Merchant </th><td>  {store_group_grosmartMerchant} </td></tr>',
        '<tr><th width="40%">Featured </th><td>  {isFeatured} </td></tr>',
        '<tr><th width="40%">Status</th><td>',
        "<tpl if=\"status == '1'\">Active</tpl>",
        "<tpl if=\"status == '0'\">Inactive</tpl>",
        "</td></tr>",
    ...
    var _storegroupsMasterStore = new Ext.data.GroupingStore({
      ...
      reader: new Ext.data.JsonReader(
        ...
        [
          ...
          "sg_primary_bt",
          "sg_additional_bt",
          "siteUrl","permalink","domain","customDomain","grosmartMerchant"
        ]
      ),
      sortInfo: {
      ...
      tbar: [
        {
          ...
          handler: function () {
            ...
            var storegroupsForm = Ext.getCmp(
              "formpanelMasterStoreGroups"
            ).getForm();
            Ext.getCmp("panelMasterStoreGroupParent").setTitle(
              "Create Store Group Details"
            );
            loadedForm = null;
  ...
  var saveStoreGroups = function () {
    ...
    if (
      !Ext.isEmpty(Ext.getCmp("store_group_name").getValue()) &&
      ...
      Ext.Ajax.request({
        ...
        params: {
          ...
          mobile: Ext.getCmp("store_group_mobile").getValue(),
          store_group_primary_businessType: Ext.getCmp(
            "store_group_primary_businessType"
          ).getValue(),
          store_group_additional_businessType: Ext.getCmp(
            "store_group_additional_businessType"
          ).getValue(),
          store_group_outofStock: Ext.getCmp(
            "store_group_outofStock"
          ).getValue(),
          store_group_grosmartMerchant: Ext.getCmp(
            "store_group_grosmartMerchant"
          ).getValue(),
          status: Ext.getCmp("comboMasterStoreGroupsStatus").getValue(),
          isFeatured: Ext.getCmp("isFeatured").getValue(),
          exclusiveMerchant: Ext.getCmp("exclusiveMerchantIds").getValue(),
          reservedMerchant: Ext.getCmp("reservedMerchantIds").getValue()
        },
        success: function (response) {
          var tmp = Ext.decode(response.responseText);
          if (tmp.success === true && tmp.valid === true) {
                ...
                .load({
                  callback: function (record, options, success) {
                    var gridPanel = Ext.getCmp(
                      "gridpanelMasterDataviewStoreGroupsdata"
                    );
            ...
            Application.RetalineSettings.ViewStoreGroups(
              tmp.data.store_group_id
            );
          } else if (tmp.success === true && tmp.valid === false) {
            Ext.Msg.alert("Notification.", tmp.message);
          } else if (tmp.success === true && tmp.img_valid === false) {
    ...
    var _mpanelforStoreGroup = new Ext.Panel({
      ...
      items: [
        ...
        new Ext.Panel({
          ...
          height: winsize.height * 0.6,
          items: [StoreGroupForm(), StoreGroupMasterDetailsView()],
          buttonAlign: "right",
          fbar: [
            {
              text: "Cancel",
  ...
  var masterPanelforBusinessCategory = function (id) {
    var _mpanelforBusinessCategory = new Ext.Panel({
      frame: false,
      hideBorders: true,
      layout: "border",
      border: false,
      title: "Business Categorys",
      id: id,
      iconCls: "my-icon448",
      items: [
        BusinessCategoryMainGrid(),
        new Ext.Panel({
          ...
          autoScroll: true,
          cls: "left_side_panel",
          id: "panelMasterBusinessCategoryParent",
          height: winsize.height * 0.6,
          items: [BusinessCategoryForm(), BusinessCategoryMasterDetailsView(),RCinBusinessCateogry()],
          buttonAlign: "right",
          fbar: [
            /*<?php if (user_access("retaline_settings", "saveBusinessCategorys")) { ?> */ {
              text: "Edit",
              cls: "left-right-buttons",
              iconCls: "edit",
              id: "buttonMasterBusinessCategoryEdit",
              icon: IMAGE_BASE_PATH + "/default/icons/mem_edit.png",
              tabIndex: 4,
              handler: function () {
                var ID = Ext.getCmp(
                  "gridpanelMasterDataviewBusinessCategorysdata"
                )
                  .getSelectionModel()
                  .getSelections()[0].data.business_category_id;
                Application.RetalineSettings.EditBusinessCategorysView(ID);
              },
            },
            {
              text: "Cancel",
              ...
              cls: "left-right-buttons",
              icon: IMAGE_BASE_PATH + "/default/icons/cancel.png",
              id: "buttonMasterBusinessCategoryCancel",
              hidden: true,
              handler: function () {
                if (
                  !Ext.isEmpty(
                    Ext.getCmp("gridpanelMasterDataviewBusinessCategorysdata")
                      .getSelectionModel()
                      .getSelections()
                  )
                ) {
                  var ID = Ext.getCmp(
                    ...
                    .getSelectionModel()
                    .getSelections()[0].data.business_category_id;
                  Application.RetalineSettings.ViewBusinessCategorys(ID);
                }
              },
            },
            {
              ...
              tabIndex: 4,
              cls: "left-right-buttons",
              id: "buttonMasterBusinessCategorySave",
              icon: IMAGE_BASE_PATH + "/default/icons/disk.png",
              hidden: true,
              handler: function () {
                saveBusinessCategorys();
              },
            } /*<?php } ?> */,
          ],
        }),
    ...
    return _mpanelforBusinessCategory;
  };
  var BusinessCategoryForm = function () {
    var _businesscategorysFormPanel = new Ext.form.FormPanel({
      id: "formpanelMasterBusinessCategorys",
      frame: false,
      border: false,
      hidden: true,
      autoHeight: true,
          ...
          items:[
            ...
            },
            {
              ...
              items: [mkCombo({
                ...
                tabIndex: 2,
                emptyText: "Set status..",
                id: "comboMasterBusinessCategorysStatus",
              })]
            }
          ]
        },
        ...
        afterrender: function () {
          if (Ext.isEmpty(Ext.getCmp("business_category_id").getValue())) {
            var recordSelected = Ext.getCmp(
              "comboMasterBusinessCategorysStatus"
            )
              .getStore()
              .getAt(0);
            Ext.getCmp("comboMasterBusinessCategorysStatus").setValue(
              recordSelected.get("id")
            );
          }
        },
    ...
    return _businesscategorysFormPanel;
  };
  var BusinessCategoryMasterDetailsView = function () {
    return new Ext.Panel({
      layout: "fit",
      region:"south",
      border: false,
      hideBorders: true,
      height: winsize.height * 0.1,
      id: "panelMasterBusinessCategorysDetailsView",
      tpl: new Ext.XTemplate(
        '<div class="details-outer">',
        '<table border="0" width="100%" class="details_view_table">',
  ...
  var BusinessCategorysMasterStore = function () {
    var _businesscategorysMasterStore = new Ext.data.GroupingStore({
      proxy: new Ext.data.HttpProxy({
        url: modURL + "&op=listBusinessCategorys",
        method: "post",
      }),
      reader: new Ext.data.JsonReader(
        {
    ...
    return _businesscategorysMasterStore;
  };
  var BusinessCategoryMainGrid = function () {
    var _businesscategorysStore = BusinessCategorysMasterStore();
    var _businesscategorysGridFilter = new Ext.ux.grid.GridFilters({
      filters: [
        {
          type: "string",
    ...
    _businesscategorysGridFilter.remote = true;
    _businesscategorysGridFilter.autoReload = true;
    var _businesscategorysmaingridPanel = new Ext.grid.GridPanel({
      region: "center",
      layout: "fit",
      frame: false,
      border: false,
      loadMask: true,
      store: _businesscategorysStore,
      iconCls: "money",
      id: "gridpanelMasterDataviewBusinessCategorysdata",
      view: new Ext.grid.GroupingView({
        forceFit: true,
        deferEmptyText: false,
        emptyText:
      ...
      plugins: [new Ext.ux.grid.GroupSummary(), _businesscategorysGridFilter],
      columns: [
        new Ext.grid.RowNumberer(),
        {
      ...
      sm: new Ext.grid.RowSelectionModel({
        singleSelect: true,
        listeners: {
          selectionchange: gridSelectionChangedbusinesscategorys,
        },
      }),
      listeners: {
        cellClick: function (grid, rowIndex, columnIndex, e) {
          ...
          if (!Ext.isEmpty(ID)) {
            Application.RetalineSettings.Cache.business_category_id = ID;
            Ext.getCmp("formpanelMasterBusinessCategorys").hide();
            Application.RetalineSettings.ViewBusinessCategorys(ID);
          }
        },
        resize: onGridResize,
        afterrender: function () {
          _businesscategorysStore.load();
        },
      },
      tbar: [
        {
          text: "Create Business Category",
          tooltip: "Create Business Category ",
          icon: "./resources/images/submenuicons/add.png",
          iconCls: "my-icon1",
          handler: function () {
            Application.RetalineSettings.BusinessCategorysAddEdit = "Add";
            var businesscategorysForm = Ext.getCmp(
              "formpanelMasterBusinessCategorys"
            ).getForm();
            Ext.getCmp("panelMasterBusinessCategoryParent").setTitle(
              "Create Business Category Details"
            );
            loadedForm = null;
            businesscategorysForm.reset();
            Ext.getCmp("business_category_name").focus(false, 100);
            /*<?php if (user_access("retaline_settings", "saveBusinessCategorys")) { ?> */
            Ext.getCmp("gridBusinesCatRetailCatList").hide();
            Ext.getCmp("buttonMasterBusinessCategoryEdit").hide();
            Ext.getCmp("buttonMasterBusinessCategorySave").show();
            /*<?php } ?> */
            Ext.getCmp("buttonMasterBusinessCategoryCancel").show();
            Ext.getCmp("formpanelMasterBusinessCategorys").show();
            Ext.getCmp("panelMasterBusinessCategorysDetailsView").hide();
            Ext.getCmp("panelMasterBusinessCategoryParent").doLayout();
            Ext.getCmp("gridBusinesCatRetailCat")
              .getStore()
              .load({
                params: {
    ...
    return _businesscategorysmaingridPanel;
  };
  var saveBusinessCategorys = function () {
    rbc_retailCategory = [];

    var store_fields = Ext.getCmp("gridBusinesCatRetailCat")
      .getSelectionModel()
      .getSelections();
    ...
    if (
      ...
      !Ext.isEmpty(Ext.getCmp("comboMasterBusinessCategorysStatus").getValue())
    ) {
      Ext.Ajax.request({
        url: modURL + "&op=saveBusinessCategorys",
        method: "POST",
        params: {
          ...
          status: Ext.getCmp("comboMasterBusinessCategorysStatus").getValue(),
        ...
        success: function (response) {
          ...
          if (tmp.success === true && tmp.valid === true) {
            ...
            if (
              ...
              Ext.getCmp("formpanelMasterBusinessCategorys").getForm().reset();
              Ext.getCmp(
                "gridpanelMasterDataviewBusinessCategorysdata"
              ).store.reload({
                params: {
              ...
              //                            Ext.getCmp('gridpanelMasterDataviewBusinessCategorysdata').selModel.getSelected().data = tmp.data;
              //                            Ext.getCmp('gridpanelMasterDataviewBusinessCategorysdata').getStore().reload();
              //                            Ext.getCmp('gridpanelMasterDataviewBusinessCategorysdata').getView().refresh();
            }
            Application.RetalineSettings.BusinessCategorysAddEdit = "";
            Application.RetalineSettings.ViewBusinessCategorys(
  ...
  var gridSelectionChangedbusinesscategorys = function (sm) {
    if (
      !Ext.isEmpty(
        Ext.getCmp("gridpanelMasterDataviewBusinessCategorysdata")
          .getSelectionModel()
      ...
      var ID = Ext.getCmp("gridpanelMasterDataviewBusinessCategorysdata")
        .getSelectionModel()
  ...
  var RCinBusinessCateogry = function(){
    var _bcRetailCategGridStore = new Ext.data.JsonStore({
      autoLoad: true,
      url: modURL + "&op=listBusinessCatRetailCat",
      method: "post",
      fields: ["business_type_id", "business_type_name", "checked"],
      remoteSort: true,
    });
    ...
    __rcGridFilter.autoReload = true;
    var _albumProjectEventGrid = new Ext.grid.GridPanel({
      id: "gridBusinesCatRetailCatList",
      region: "north",
      height: 385,
      width: winsize.width * 0.38,
      frame: true,
  ...
  var mapMerchantStore = function (type) {
    var _store = new Ext.data.GroupingStore({
      proxy: new Ext.data.HttpProxy({
        url: modURL + "&op=listMapppedStoreGroups",
        method: "post",
      }),
      reader: new Ext.data.JsonReader(
        {
        ...
        [
          ...
          "sg_primary_bt",
          "sg_additional_bt",
          "siteUrl","permalink","domain","customDomain"
        ]
      ),
      sortInfo: {
        field: "store_group_id",
      ...
      listeners: {
        load: function () {
          loadCount++;
          if (loadCount == 1) {
            Ext.getCmp("mapMerchantGrid").getSelectionModel().selectRow(0);
          }
        },beforeload: function () {
  ...
  var mapMerchantGrid = function (type) {
    var _mapMerchantGridFilter = new Ext.ux.grid.GridFilters({
      filters: [{
        type: "string",
        dataIndex: "store_group_name",
      },
    ...
    _mapMerchantGridFilter.remote = true;
    _mapMerchantGridFilter.autoReload = true;
    var _masterStore = mapMerchantStore(type);
    var _gridPanel = new Ext.grid.GridPanel({
      store: _masterStore,
      region: "center",
      layout: "fit",
      ...
      plugins: [_mapMerchantGridFilter],
      id: "mapMerchantGrid",
      columns: [
        new Ext.grid.RowNumberer(),  
        {
              ...
              items: [
                {
                  ...
                  handler: function (grid, rowIndex, colIndex) {
                    ...
                    switch (type) {
                      case 1:
                          addMerchant(record, 'exclusiveMerchant', 'exclusiveMerchantIds');
                          break;
                      case 2:
                          addMerchant(record, 'reservedMerchant', 'reservedMerchantIds');
                          break;
                  }
                ...
                },
                {
                  ...
                  handler: function (grid, rowIndex, colIndex) {
                    ...
                    switch (type) {
                        case 1:
                            removeMerchant(record, 'exclusiveMerchant', 'exclusiveMerchantIds');
                            break;
                        case 2:
                            removeMerchant(record, 'reservedMerchant', 'reservedMerchantIds');
                            break;
                    }
                  },
                },
      ...
      tbar: [{
                ...
                tabIndex: 703,
                handler: function () {
                  var storefilter = Ext.getCmp('mapMerchantGrid');
                  if(!Ext.isEmpty(Ext.getCmp('merchantSearchParameter').getValue()))
                  activateFilter(storefilter, 'store_group_name', Ext.getCmp('merchantSearchParameter').getValue());
    ...
    var addMerchant = function (record, nameCmpId, idCmpId) {
      console.log('add');
      var nameCmp = Ext.getCmp(nameCmpId);
      var idCmp   = Ext.getCmp(idCmpId);

      // Get current values (split into arrays, fallback to empty)
      var names = nameCmp.getValue() ? nameCmp.getValue().split(',') : [];
      var ids   = idCmp.getValue() ? idCmp.getValue().split(',') : [];
console.log('names',names);
console.log('ids',ids);
      // Add if not already present
      var newId = record.get("store_group_id").toString();
      if (!ids.includes(newId)) {
          names.push(record.get("store_group_name"));
          ids.push(newId);
      }
console.log('names',names);
console.log('ids',ids);
      // Update fields
      nameCmp.setValue(names.join(','));
      idCmp.setValue(ids.join(','));
  };
  var removeMerchant = function (record, nameCmpId, idCmpId) {
      var nameCmp = Ext.getCmp(nameCmpId);
      var idCmp   = Ext.getCmp(idCmpId);
      ...
      var removeId = record.get("store_group_id").toString();

      // Find index of ID to remove
      var index = ids.indexOf(removeId);
      if (index !== -1) {
  ...
  var masterPanelforGroSmartMerchant = function (id) {
    var _mpanelforGroSmartMerchant = new Ext.Panel({
      frame: false,
      hideBorders: true,
      layout: "border",
      border: false,
      title: "GroSmart Merchants",
      id: id,
      iconCls: "my-icon448",
      items: [GroSmartMerchantMainGrid()],
    });
    return _mpanelforGroSmartMerchant;
  };    
  var GroSmartMerchantStore = function () {
    var _groSmartMerchantMasterStore = new Ext.data.GroupingStore({
      proxy: new Ext.data.HttpProxy({
        url: modURL + "&op=listStoreGroups",
        method: "post",
      }),
      ...
      listeners: {
        load: function () {
          Ext.getCmp("gridpanelGroSmartMerchantdata")
            .getView()
            .refresh();
          Ext.getCmp("gridpanelGroSmartMerchantdata")
            .getSelectionModel()
        ...
        beforeload:function(){
          this.baseParams.grosmartMerchant = 1;
        }
      },
    });
    return _groSmartMerchantMasterStore;
  };
  var GroSmartMerchantMainGrid = function () {
    var _groSmartMerchantStore = GroSmartMerchantStore();
    var _groSmartMerchantGridFilter = new Ext.ux.grid.GridFilters({
      filters: [
        {
          type: "string",
          dataIndex: "store_group_name",
    ...
    _groSmartMerchantGridFilter.remote = true;
    _groSmartMerchantGridFilter.autoReload = true;
    var _groSmartMerchantmaingridPanel = new Ext.grid.GridPanel({
      region: "center",
      layout: "fit",
      frame: false,
      border: false,
      loadMask: true,
      store: _groSmartMerchantStore,
      iconCls: "money",
      id: "gridpanelGroSmartMerchantdata",
      view: new Ext.grid.GroupingView({
        forceFit: true,
        deferEmptyText: false,
        emptyText:
      ...
      plugins: [new Ext.ux.grid.GroupSummary(), _groSmartMerchantGridFilter],
      columns: [
        new Ext.grid.RowNumberer(),
        {
          header: "SG ID",
      ...
      listeners: {
        ...
        resize: onGridResize,
        afterrender: function () {
          _groSmartMerchantStore.load();
        },
      },
      tbar: [
      ],
    });
    return _groSmartMerchantmaingridPanel;
  };
  return {
    Cache: {},
    initfaq: function () {
          ...
          success: function (response, action) {
            ...
            if (tmp.success === true && tmp.valid === true) {
              ...
              if (Application.RetalineSettings.NotificationAddEdit == "Add") {
                ...
                Ext.getCmp(
                  "appNotificationGridPanel"
                ).selModel.getSelected().data = tmp.data;
                Ext.getCmp("appNotificationGridPanel").getStore().load();
              }
              Application.RetalineSettings.NotificationAddEdit = "";
              Application.RetalineSettings.ViewNotificationMode(
                tmp.data.notification_id
              );
            } else if (tmp.success === true && tmp.valid === false) {
              Ext.Msg.alert("Notification.", tmp.message);
            } else if (tmp.success === true && tmp.img_valid === false) {
        ...
        success: function (res) {
          ...
          if (tmp.success === true) {
            var visualsDescPanel = Ext.getCmp(
              "xtemplatenotificationViewDetails"
            );
            visualsDescPanel.update(tmp);
          }
          Ext.getCmp("notificationPanel").doLayout();
    ...
    ViewpaymentTermsMode: function () {
      ...
      Ext.Ajax.request({
        ...
        success: function (res) {
          ...
          if (tmp.success === true) {
            var visualsDescPanel = Ext.getCmp(
              "xtemplatepaymentTermsViewDetails"
            );
            visualsDescPanel.update(tmp);
          }
          Ext.getCmp("paymentTermsPanel").doLayout();
                                ...
                                buttons: [
                                  {
                                    ...
                                    id: "id_clr",
                                    handler: function () {
                                      Application.RetalineCurrentStock.displayBarcodeDetails(
                                        Ext.getCmp("bcentry").getValue()
                                      );
                                    },
                                  },
                                ],
    ...
    ViewStoreGroups: function () {
      ...
      Ext.Ajax.request({
        ...
        success: function (res) {
          ...
          if (tmp.success === true) {
            var visualsDescPanel = Ext.getCmp(
              "panelMasterStoreGroupsDetailsView"
            );
            visualsDescPanel.update(tmp);
          }
          Ext.getCmp("panelMasterStoreGroupParent").doLayout();
    ...
    EditStoreGroupsView: function () {
      ...
      if (!Ext.isEmpty(arguments[0])) {
        ...
        storegroupsForm.load({
          ...
          success: function (form, action) {
            var tmp = Ext.decode(action.response.responseText);
            Ext.getCmp('store_group_additional_businessType').getStore().baseParams.primaryBt = tmp.data.store_group_primary_businessType;
            
            Ext.getCmp('store_group_additional_businessType').getStore().load();
            Ext.getCmp('store_group_additional_businessType').setValue(tmp.data.store_group_additional_businessType);
            Ext.getCmp('store_group_additional_businessType').setRawValue(tmp.data.sg_additional_bt);
            Ext.getCmp('exclusiveMerchantIds').setValue(tmp.data.exclusiveMerchant);
            Ext.getCmp('reservedMerchantIds').setValue(tmp.data.reservedMerchant);
            Ext.getCmp('exclusiveMerchant').setValue(tmp.data.exclusiveMerchantName);
            Ext.getCmp('reservedMerchant').setValue(tmp.data.reservedMerchantName);
    ...
    initBusinessCategory: function () {
      var _businesscategoryPanelId = "panelMasterMainBusinessCategory";
      var _masterPanelBusinessCategory = Ext.getCmp(_businesscategoryPanelId);
      if (Ext.isEmpty(_masterPanelBusinessCategory)) {
        _masterPanelBusinessCategory = masterPanelforBusinessCategory(
          _businesscategoryPanelId
        );
        Application.UI.addTab(_masterPanelBusinessCategory);
        _masterPanelBusinessCategory.doLayout();
      } else {
        Application.UI.addTab(_masterPanelBusinessCategory);
      }
    },
    ViewBusinessCategorys: function () {
      var business_category_id = arguments[0];
      /*<?php if (user_access("retaline_settings", "saveBusinessCategorys")) { ?> */
      Ext.getCmp("buttonMasterBusinessCategoryEdit").show();
      Ext.getCmp("buttonMasterBusinessCategorySave").hide();
      /*<?php } ?> */
      Ext.getCmp("buttonMasterBusinessCategoryCancel").hide();
      Ext.getCmp("formpanelMasterBusinessCategorys").hide();
      Ext.getCmp("panelMasterBusinessCategorysDetailsView").show();
      Ext.getCmp("panelMasterBusinessCategoryParent").doLayout();
      Ext.getCmp("panelMasterBusinessCategoryParent").setTitle(
        "View Business Category Details"
      );
      Ext.Ajax.request({
        url: modURL + "&op=businesscategorysdetailsView",
        method: "POST",
        ...
        success: function (res) {
          ...
          if (tmp.success === true) {
            var visualsDescPanel = Ext.getCmp(
              "panelMasterBusinessCategorysDetailsView"
            );
            visualsDescPanel.update(tmp);
          }
          Ext.getCmp("panelMasterBusinessCategoryParent").doLayout();
          Ext.getCmp("gridBusinesCatRetailCatList").show();
          Ext.getCmp("gridBusinesCatRetailCatList")
          .getStore()
          .load({
            params: {
    ...
    EditBusinessCategorysView: function () {
      Application.RetalineSettings.BusinessCategorysAddEdit = "Edit";
      Ext.getCmp("panelMasterBusinessCategoryParent").doLayout();
      Ext.getCmp("panelMasterBusinessCategoryParent").setTitle(
        "Edit Business Category Details"
      );
      Ext.getCmp("formpanelMasterBusinessCategorys").show();
      Ext.getCmp("panelMasterBusinessCategorysDetailsView").hide();
      /*<?php if (user_access("retaline_settings", "saveBusinessCategorys")) { ?> */
      ...
      if (!Ext.isEmpty(arguments[0])) {
        ...
        businesscategorysForm.load({
          params: {
            business_category_id: arguments[0],
            apikey: _SESSION.apikey,
            tstamp: t_stamp,
          },
          url: modURL + "&op=businesscategorys_form_load",
          waitMsg: "Loading...",
          success: function (form, action) {
            var tmp = Ext.decode(action.response.responseText);
          },
    ...
    },mapMerchants : function(type){
      var mapMerchantWindow = new Ext.Window({
          id: "windowFormapMerchant",
          iconCls: "",
          shadow: false,
          frame: true,
        ...
        mapMerchantWindow.doLayout();
        mapMerchantWindow.show();
        mapMerchantWindow.center();
    },initGroSmartMerchant: function () {
      var _groSmartMerchantPanelId = "panelMasterGroSmartMerchant";
      var _masterPanelGroSmartMerchant = Ext.getCmp(_groSmartMerchantPanelId);
      if (Ext.isEmpty(_masterPanelGroSmartMerchant)) {
        _masterPanelGroSmartMerchant = masterPanelforGroSmartMerchant(_groSmartMerchantPanelId);
        Application.UI.addTab(_masterPanelGroSmartMerchant);
        _masterPanelGroSmartMerchant.doLayout();
      } else {
        Application.UI.addTab(_masterPanelGroSmartMerchant);
      }
    }
