...
switch ($op) {
    ...
    case 'getBranchDestination':
        ...
        $qry = $db->getMulipleData("SELECT br_ID,CONCAT(br_Name,' - ',branch_shortname) AS br_Name FROM finascop_branch where br_status = 'Active' AND br_type <> 1 {$cond}", true);
    ...
    case 'getItemName':
        ...
        if ($_POST['sourceBranch'] > 0) {
            ...
            $searchQuery .= " AND branch_id = {$_POST['sourceBranch']} AND stit_ParentItemId = 0 ";
        }
    ...
    case 'saveTransferRequest':
        ...
        $uid_max = getNewTRNumber($_POST['fstr_source']);
...
//                }
        ...
        $listQuery = "SELECT fstr_id,fstr_uid,fstr_source,fstr_destination,fstr_status,"
                ...
                . "fstr_initiatedBy,(SELECT CONCAT(br_Name,' - ',branch_shortname) FROM finascop_branch WHERE br_ID = fstr_initiatedBy) as initiatedBranch,"
                . "(SELECT CONCAT(br_Name,' - ',branch_shortname) FROM finascop_branch where br_ID = fstr_source) as sourcename,"
                . "(SELECT CONCAT(br_Name,' - ',branch_shortname) FROM finascop_branch where br_ID = fstr_destination) as branch  FROM finascop_stock_transfer_request INNER JOIN finascop_stock_transfer_request_status ON fstrs_id = fstr_status   {$search} {$where} ORDER BY {$sort} {$dir} limit $start,$limit";
...
//            $data['fsto_uid'] = $maxFstId;
            ...
            $uid_max = getNewTONumber($order_det['fstr_source']);
...
////
                        ...
                        $itemPriceDetails = $db->getFromDB("SELECT mrp,selling_price,fpod_poLandingCostleastSKU,fpod_leastSKUepr FROM finascop_stock_branch_inventory WHERE stit_id = {$fstr_ItemId[$i]} AND branch_id = {$order_det['fstr_source']} and fpod_leastSKUmrp = {$fstr_ItemMRP}", true);
...
//        }
    ...
    case 'listRetalineStockIndent':
        ...
        if (isset($filter)) {
            foreach ($filter as $key => $field) {
                switch ($field['data']['type']) {
                    case 'list':
                        if ($field['field'] == 'status_name') {
                            if ($field['data']['value'] == 'Indent Requested') {
                                //    $field['data']['value'] = 2;
                                $fiterItem = 1;
                                $search .= " and (fstr_status = {$fiterItem}) ";
                            } else if ($field['data']['value'] == 'Partially Attended') {
        ...
        if ($_SESSION['admin']->br_PyramidLevel == 1)
            $where = " AND fstr_source = {$br_ID} OR fstr_destination = {$br_ID} ";
        else
            $where = " AND fstr_source = {$_SESSION['admin']->finascop_current_branch_id} OR fstr_destination = {$_SESSION['admin']->finascop_current_branch_id} ";
        $countQuery = "SELECT COUNT(*) FROM finascop_stock_indent  INNER JOIN finascop_stock_transfer_request_status ON fstrs_id = fstr_status {$search} {$where}";

        $listQuery = "SELECT fstr_id,fstr_uid,fstr_source,fstr_destination,fstr_status,"
                . "CASE WHEN fstr_type=1 THEN 'Transfer Invoked' WHEN fstr_type=2 THEN 'Stock Requested' WHEN fstr_type=3 THEN 'Sales Order'  ELSE 'Indent Requested' END AS fstr_type,"
                . "fssi_status AS status_name,"
                . "DATE_FORMAT(fstr_createdOn,'%d %M %Y') as fstr_createdOn,DATE_FORMAT(rsi_date,'%d %M %Y') as rsi_date,"
                . "fstr_initiatedBy,(SELECT CONCAT(br_Name,' - ',branch_shortname) FROM finascop_branch WHERE br_ID = fstr_initiatedBy) as initiatedBranch,"
                . "(SELECT CONCAT(br_Name,' - ',branch_shortname) FROM finascop_branch where br_ID = fstr_source) as sourcename,"
                . "(SELECT CONCAT(br_Name,' - ',branch_shortname) FROM finascop_branch where br_ID = fstr_destination) as branch  FROM finascop_stock_indent INNER JOIN finascop_stock_transfer_request_status ON fstrs_id = fstr_status   {$search} {$where} ORDER BY {$sort} {$dir} limit $start,$limit";

        $db->printGridJson($countQuery, $listQuery);
        break;
    case 'saveStockIndent':
        $uuid = $_POST['uuid'];
        $fstr_id = $_POST['fstr_id'];
        $fstr['rsi_date'] = date('Y-m-d', strtotime($_POST['rsi_date']));
        $fstr['fstr_source'] = $_POST['fstr_source'];
        $fstr['fstr_destination'] = $_POST['fstr_destination'];
        $fstr['fstr_createdOn'] = date("Y-m-d H:i");
        $fstr['fstr_createdBy'] = $_SESSION['admin']->UserId;
        ...
        $date = date("Y-m-d H:i");
        $tdy = date("Y-m-d") . " 00:00";
        $maxId = $db->getItemSafe("select right(fstr_uid,3)*1 as fstr_uid  from `finascop_stock_indent` where `fstr_source` = ? and `fstr_createdOn` between '{$tdy}' and '{$date}' order by `fstr_id` desc limit 1", "s", [$_POST['fstr_source']]);
        $br_key = $db->getItemSafe("SELECT br_key FROM finascop_branch WHERE br_ID=?", "s", [$_POST['fstr_source']]);
        $uid_max = 'SIQ' . date('ymd') . $br_key . str_pad(($maxId + 1), 3, '0', STR_PAD_LEFT);
        $fstr['fstr_uid'] = $uid_max;
        $status = $db->perform('finascop_stock_indent', $fstr);
        $lastId = $db->insert_id();
        $fstrd['fstrd_status'] = 1;
        $fstrd['fstr_id'] = $lastId;
        $fstrd['fstrd_createdOn'] = date("Y-m-d H:i");
        $fstrd['fstrd_createdBy'] = $_SESSION['admin']->Finascop_UserId;
        $status = $db->perform('finascop_stock_indent_details', $fstrd, 'update', " uuid = '{$uuid}'");
        $status = $db->query('commit');
        if ($status == 1) {
        ...
        break;
    case 'addItemstoStockIndent':
        $uuid = $_POST['uuid'];
        $fstr_id = $_POST['fstr_id'];
        $fstrd['fstr_ItemId'] = $_POST['fstr_ItemId'];
        $fstrd['fstr_RequiredItemQty'] = $_POST['fstr_RequiredItemQty'];
        if($_POST['fstr_ClosingItemQty'] > 0){
            $fstrd['fstr_ClosingItemQty'] = $_POST['fstr_ClosingItemQty'];
        }

        $fstrd['fstr_ItemUnits'] = $_POST['fstr_StockItemUnits'];
        $fstrd['fstr_leastSKUCount'] = $_POST['leastSKUCountfstr'];
        $fstrd['least_package_type_id'] = $_POST['fstrleast_package_type_id'];
        $count = $db->getItemSafe("SELECT COUNT(*) FROM finascop_stock_indent_details WHERE uuid = '{$uuid}' AND fstr_ItemId = ?", "i", [$_POST['fstr_ItemId']]);
        $db->query('begin');
        if ($count > 0) {
            $fstrd['fstrd_createdOn'] = date("Y-m-d H:i");
            $fstrd['fstrd_createdBy'] = $_SESSION['admin']->Finascop_UserId;
            $status = $db->perform('finascop_stock_indent_details', $fstrd, 'update', " uuid = '{$uuid}' AND fstr_ItemId = {$_POST['fstr_ItemId']}");
        } else {
            $fstrd['uuid'] = $uuid;
            $fstrd['fstrd_createdOn'] = date("Y-m-d H:i");
            $fstrd['fstrd_createdBy'] = $_SESSION['admin']->Finascop_UserId;
            $status = $db->perform('finascop_stock_indent_details', $fstrd);
        }
        $status = $db->query('commit');
        if ($status == 1) {
            $msg = "'Items added to indent.'";
    ...
    case 'listStockIndentItemStore':
        ...
        $countQuery = "SELECT COUNT(1) from " . FINASCOP_DB . "finascop_stock_indent_details where {$filter_part} {$cond}";
        $listQuery = "SELECT uuid,fstr_id,fstrd_id,fstr_ItemId,fstr_RequiredItemQty,fstrd_status,fstr_ClosingItemQty,(SELECT stit_SKU FROM finascop_stock_itemmaster where stit_ID = fstr_ItemId) as fstr_ItemName,"
                . "fstr_ItemUnits,fstr_leastSKUCount,least_package_type_id,(select package_type_name from mypha_productpackage_type WHERE package_type_id = least_package_type_id) as leastSkuUnit,"
                . "(select package_type_name from mypha_productpackage_type WHERE package_type_id = fstr_ItemUnits) as unitName "
                . "from " . FINASCOP_DB . "finascop_stock_indent_details WHERE {$filter_part} {$cond} ORDER BY $rec_sort $rec_sort_dir ";
        $db->printGridJson($countQuery, $listQuery);
        break;
    case 'listViewStockIndentData':
        $limit = isset($_POST['limit']) ? $_POST['limit'] : 20;
        $start = isset($_POST['start']) ? $_POST['start'] : 0;
        $sort = empty($sort) ? 'fstrd_id' : $sort;
        $dir = empty($dir) ? 'ASC' : $dir;
        ...
        $countQry = "SELECT COUNT(*) FROM finascop_stock_indent_details  {$search} AND fstr_id = " . intval($_POST['fstr_id']) . "";

        $listQry = "SELECT fstr_id,fstrd_id,fstr_ItemId,fstr_ApprovedItemQty,fstr_RequiredItemQty,fstrd_status,fstr_ItemUnits,fstr_leastSKUCount,least_package_type_id,fstr_ClosingItemQty,"
                . "CASE WHEN fstrd_status=1 THEN 'Requested' WHEN fstrd_status=2 THEN 'Ordered' WHEN fstrd_status=3 THEN 'Deleted' WHEN fstrd_status=4 THEN 'Packed' "
                . "WHEN fstrd_status=5 THEN 'Partially Received' WHEN fstrd_status=6 THEN 'Received' ELSE 'Not Saved' END AS status_name FROM finascop_stock_indent_details {$search} AND fstr_id ={$_POST['fstr_id']} ORDER BY {$sort} {$dir} ";
        $datas = $db->getMulipleData($listQry, true);
        $fstr_source = $db->getItemSafe("SELECT fstr_source FROM finascop_stock_indent WHERE fstr_id = ?", "i", [$_POST['fstr_id']]);
        $sourcePyramid = $db->getItemFromDB("SELECT br_PyramidLevel FROM finascop_branch where br_ID = {$fstr_source}");
        $resCount = count($datas);
        $count = $db->getItemFromDB($countQry);
        //print_r($datas);
    ...
    case 'deleteSIItem':
        $id = $_POST['fstrd_id'];
        $del_query = "DELETE FROM finascop_stock_indent_details WHERE fstrd_id=" . $id;
        $db->query($del_query);
        $status = $db->query('commit');
        if ($status) {
    ...
    case 'getItemNameSI':
        ...
        //finascop_getjsonkeyarray("SELECT stit_ID,stit_itemName,stit_SKU FROM finascop_stock_itemmaster where 1=1 {$searchQuery} ");
        $qry = $db->getMulipleData("SELECT stit_ID,stit_itemName,stit_SKU,cs_package_type_name,{$packageName} as packageName "
                . "FROM finascop_stock_itemmaster fsi where 1=1 AND directPurchase = 1 {$searchQuery}", true);
        if (!empty($qry)) {
            echo json_encode($qry);
        } else
            echo [];
        break;
    case 'getSourceBranch':
        $destination = $_POST['destination'];
        $packTypes = $db->getFromDB("SELECT br_cpd,CONCAT(br_Name,' - ',branch_shortname) AS br_Name FROM finascop_branch WHERE br_ID = {$destination}", true);
        $result['br_cpd'] = $packTypes['br_cpd'];
        $result['success'] = true;
        if (!empty($result)) {
            echo json_encode($result);
        }
