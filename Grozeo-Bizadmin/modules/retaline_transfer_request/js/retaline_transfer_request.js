...
//                }
        ...
        var grid_panel = new Ext.grid.GridPanel({
            ...
            columns: [new Ext.grid.RowNumberer(),
                {
                    header: 'Branch Indent No',
                    id: 'branch_name_auto_exptr',
                    sortable: true,
                    hideable: false,
                    dataIndex: 'fstr_uid',
                    tooltip: 'Branch Indent No'
                },
                {
                    header: 'Created Date',
                    sortable: true,
            ...
            tbar: [{
                    xtype: 'button',
                    text: 'Create Branch Indent',
                    tooltip: 'Create Branch Indent',
                    iconCls: 'add',
                    id: 'addnewTR',
                    handler: function () {
                        var t = new Date();
...
//    });
        ...
        var stockrequestActionMenu = new Ext.menu.Menu({
            items: [{
                ...
                },
                {
                    ...
                    handler: function () {
                        var fstr_id = Ext.getCmp('retalineTransferRequest_main_panel').getSelectionModel().getSelections()[0].data.fstr_id;
                        var sourcename = Ext.getCmp('retalineTransferRequest_main_panel').getSelectionModel().getSelections()[0].data.sourcename;
                        Application.RetalinTransferRequest.AddNewTransferWindow(fstr_id, sourcename);
                    }
                }]
        });
            ...
            listeners: {
                beforeload: function () {
                    ...
                    if (_SESSION.br_PyramidLevel == 4) {
                        ...
                        Ext.Ajax.request({
                            url: modURL + '&op=getSourceBranch',
                            params: {destination: _SESSION.finascop_current_branch_id},
                            ...
                            success: function (response, options) {
                                ...
                                if (tmp.success === true) {
                                    if (tmp.br_cpd > 0) {
                                        Ext.getCmp("fstr_source").setValue(tmp.br_cpd);
                                        Ext.getCmp("fstr_source").setHideTrigger(true);
                                        Ext.getCmp("fstr_source").editable = false;
                                        ...
                                        Ext.getCmp('fstr_ItemId').getStore().load({
                                            params: {
                                                sourceBranch: tmp.br_cpd
            }
        });
                                        Ext.getCmp("fstr_destination").setValue(_SESSION.finascop_current_branch_id);
                                        Ext.getCmp("fstr_destination").setHideTrigger(true);
                                        Ext.getCmp("fstr_destination").editable = false;
                                    }
                                } else {
                                    Ext.MessageBox.alert('Error', response.responseText);
        ...
        var store = new Ext.data.JsonStore({
            ...
            listeners: {
                beforeload: function () {
                    ...
                    Ext.getCmp("fstr_source").setValue(_SESSION.finascop_current_branch_id);
                    Ext.getCmp('fstr_destination').getStore().load({
                        params: {
                    ...
                    Ext.getCmp('fstr_ItemId').getStore().load({
                        params: {
                            sourceBranch: _SESSION.finascop_current_branch_id
            }
        });
    ...
    var siItemStore = function () {
        var store = new Ext.data.JsonStore({
            fields: ['stit_ID', 'stit_itemName', 'stit_SKU', 'packageName', 'id', 'fsbg_id', 'fpod_leastSKUmrp'],
            url: modURL + '&op=getItemNameSI',
            autoLoad: true,
            method: 'post'
        });
        return store;
...
//                        }
    ...
    var retalineStockIndentGrid = function (id) {
        var branch_store = stockIndentStore();
        ...
        var grid_panel = new Ext.grid.GridPanel({
                ...
                },
                {
                    ...
                    listeners: {
                        click: function (a, grid, rowindex, e) {
                            var record = grid.store.getAt(rowindex);
                            grid.getSelectionModel().selectRow(rowindex);
                            var fstr_status = Ext.getCmp('retalineStockIndent_main_panel').getSelectionModel().getSelections()[0].data.fstr_status;
                            stockIndentActionMenu(e, fstr_status)
                        }
                    }
                }
            ],
            ...
            listeners: {
                ...
                afterrender: function () {
                    if (_SESSION.br_PyramidLevel == 1) {
                        Ext.getCmp("comboxbranchnamesi").show();
                        Ext.getCmp("siBranchDataeditCS").hide();
                    } else {
                        Ext.getCmp("comboxbranchnamesi").hide();
                        Ext.getCmp("siBranchDataeditCS").show();
                        Ext.getCmp("siBranchDataeditCS").setValue(_SESSION.current_branch);
                    }
                }
            },
            tbar: [{
                    xtype: 'button',
                    text: 'Create Stock Indent',
                    tooltip: 'Create Stock Indent',
                    iconCls: 'add',
                    handler: function () {
                        var t = new Date();
                        var t_stamp = t.format("YmdHis");
                        Ext.Ajax.request({
                            ...
                            success: function (res) {
                                ...
                                console.log("temp is -", tmp);
                                var uid = tmp.uid;
                                Application.RetalinTransferRequest.Cache.siuuid = uid;
                                createRetalineStockIndent(0);
                            }
                        });
                ...
                {
                    xtype: 'textfield',
                    id: 'siBranchDataeditCS',
                    name: 'siBranchDataeditCS',
                    anchor: '98%',
                    tabIndex: 1,
                    maxLength: 20,
                    setReadOnly: true,
                    ...
                    xtype: 'combo',
                    id: 'comboxbranchnamesi',
                    name: 'comboxbranchnamesi',
                    mode: 'local',
                    typeAhead: true,
                    forceSelection: true,
                    emptyText: 'Select Branch',
                    ...
                    displayField: 'br_Name',
                    valueField: 'br_ID',
                    hiddenName: 'comboxbranchnamesi',
                    listeners: {
                        select: function () {
                            Ext.getCmp('retalineStockIndent_main_panel').getStore().removeAll();
                        }
                    }
                }, {
                    xtype: 'button',
                    ...
                    id: 'idshowbtntr',
                    handler: function () {
                        var branchName = Ext.getCmp('comboxbranchnamesi').getValue();
                        Ext.getCmp('retalineStockIndent_main_panel').getStore().load({
                            params: {
                                branchName: branchName
                            }
                        });
    ...
    var stockIndentStore = function () {
        var store = new Ext.data.JsonStore({
            method: 'post',
            proxy: new Ext.data.HttpProxy({
                url: modURL + '&op=listRetalineStockIndent',
                method: 'post'
            }),
            fields: ['fstr_id', 'fstr_uid', 'fstr_source', 'fstr_destination', 'fstr_status', 'fstr_createdOn', 'sourcename', 'branch', 'status_name', 'fstr_type', 'initiatedBranch', 'fstr_initiatedBy', 'rsi_date'],
            totalProperty: 'totalCount',
            root: 'data',
            remoteSort: true,
            autoLoad: false,
            listeners: {
                beforeload: function () {
                    this.baseParams.branchName = Ext.getCmp('comboxbranchnamesi').getValue();
                }
            }
        });
        store.setDefaultSort('fstr_createdOn', 'DESC');
    ...
    var createRetalineStockIndent = function (fstr_id) {
        var resultWindow = new Ext.Window({
            id: "windowRetalineStockIndent",
            iconCls: 'vender-items',
            shadow: false,
            height: 600,
            width: 700,
            title: 'Create New Stock Indent',
            layout: 'border',
            resizable: false,
            plain: true,
            constrain: true,
            ...
            items: [{
                    ...
                    layout: 'fit',
                    height: 200,
                    items: createRetalineStockIndentform(fstr_id)
                }, {
                    region: 'south',
                    layout: 'fit',
                    height: 400,
                    items: createRetalineStockIndentItemStockGrid(fstr_id)
                }],
            buttons: [
                {
                    icon: IMAGE_BASE_PATH + '/default/icons/finascop_cancel.png',
                    text: 'Cancel',
                    handler: function () {
                        Ext.getCmp('windowRetalineStockIndent').close();
                        Ext.getCmp('retalineStockIndent_main_panel').getStore().load();
                    }
                    ...
                    handler: function () {
                        if (!Ext.isEmpty(Ext.getCmp('fstr_source').getValue()) &&
                            ...
                            Ext.Ajax.request({
                                url: modURL + '&op=saveStockIndent',
                                method: 'POST',
                                params: {
                                    uuid: Application.RetalinTransferRequest.Cache.siuuid,
                                    fstr_id: fstr_id,
                                    fstr_source: Ext.getCmp('fstr_source').getValue(),
                                    fstr_destination: Ext.getCmp('fstr_destination').getValue(),
                                    rsi_date: Ext.getCmp('rsi_date').getValue()
                                },
                                success: function (response) {
                                    var tmp = Ext.decode(response.responseText);
                                    if (tmp.success === true && tmp.valid === true) {
                                        Application.example.msg('Success', tmp.msg);
                                        Application.RetalinTransferRequest.Cache.siuuid = '';
                                        Ext.getCmp('retalineStockIndent_main_panel').getStore().reload();
                                        resultWindow.close();
                                    } else if (tmp.success === true && tmp.valid === false) {
                                        Ext.Msg.alert("Notification.", tmp.message);
                                    } else if (tmp.success === true && tmp.img_valid === false) {
    ...
    var createRetalineStockIndentform = function (fstr_id) {

        var t = new Date();
        var t_stamp = t.format("YmdHis");
        var itemsStore = siItemStore();
        var sourceStore = rtrSourceStore(1);
        var branchStore = rtrBranchStore(0);
        var poItemStockformPanel = new Ext.form.FormPanel({
            height: 200,
...
//                                        },
                            ...
                            items: [
                                {
                                    ...
                                    id: 'fstr_ClosingItemQty',
                                    name: 'fstr_ClosingItemQty',
                                    allowBlank: false,
                                    tabIndex: 703,
                                    anchor: '97%',
                                    listeners: {
                            ...
                            items: [{
                                    ...
                                    handler: function () {
                                        ...
                                        if (!Ext.isEmpty(Ext.getCmp('fstr_ItemId').getValue()) &&
                                            ...
                                            Ext.Ajax.request({
                                                url: modURL + '&op=addItemstoStockIndent',
                                                method: 'POST',
                                                params: {
                                                    uuid: Application.RetalinTransferRequest.Cache.siuuid,
                                                    fstr_id: fstr_id,
                                                    ...
                                                    fstrleast_package_type_id: Ext.getCmp('fstrleast_package_type_id').getValue(),
                                                    fstr_ItemMRP: Ext.getCmp('fstr_ItemMRP').getValue(),
                                                    fstr_ClosingItemQty: Ext.getCmp('fstr_ClosingItemQty').getValue(),
                                                },
                                                success: function (response) {
                                                    var tmp = Ext.decode(response.responseText);
                                                    if (tmp.success === true && tmp.valid === true) {
                                                        Application.example.msg('Success', tmp.msg);
                                                        Ext.getCmp('retalineStockIndentItemGridPanel').getStore().load({
                                                            params: {
                                                                uuid: Application.RetalinTransferRequest.Cache.uuid,
                                                                fstr_id: fstr_id
                                                            }
                                                        ...
                                                        Ext.getCmp('fstr_ItemId').reset();
                                                        Ext.getCmp('fstr_ClosingItemQty').reset();
                                                        Ext.getCmp('fstr_RequiredItemQty').reset();
                                                        Ext.getCmp('fstr_StockItemUnits').reset();
                                                        Ext.getCmp('leastSKUCountfstr').reset();
                                                        Ext.getCmp('leastSKUDisplayfstr').reset();
    ...
    var createRetalineStockIndentItemStockGrid = function (fstr_id) {

        var retalineTransferRequestItemGridStore = retalineStockIndentItemStore(fstr_id);
        var poStockEntryGrid = new Ext.grid.GridPanel({
            store: retalineTransferRequestItemGridStore,
            frame: false,
            border: true,
            ...
            autoScroll: true,
            plugins: [],
            id: 'retalineStockIndentItemGridPanel',
            iconCls: 'finascop_dataentry',
            loadMask: true,
            columns: [
                {
                    ...
                    items: [
                        {
                            ...
                            handler: function (grid, rowIndex, colIndex) {
                                var record = grid.store.getAt(rowIndex);
                                deleteItemStockIndent(record.get('fstrd_id'));
                            }
                        }
                    ]
    ...
    var stockIndentActionMenu = function (e, fstr_status) {
        var stockrequestActionMenu = new Ext.menu.Menu({
            items: [{
                    text: "View",
                    handler: function () {
                        var fstr_id = Ext.getCmp('retalineStockIndent_main_panel').getSelectionModel().getSelections()[0].data.fstr_id;
                        var toBranch = Ext.getCmp('retalineStockIndent_main_panel').getSelectionModel().getSelections()[0].data.branch;
                        Application.RetalinTransferRequest.viewStockIndentItems(fstr_id, toBranch);
                    }
                }]
        });
        stockrequestActionMenu.showAt(e.getXY());
    };
    var retalineStockIndentItemStore = function (fstr_id) {
        var purchase_invoice_store = new Ext.data.JsonStore({
            method: 'post',
            url: modURL + '&op=listStockIndentItemStore',
            fields: ['uuid', 'fstr_id', 'fstrd_id', 'fstr_ItemId', 'fstr_RequiredItemQty', 'fstrd_status', 'fstr_ItemName', 'fstr_ItemUnits', 'fstr_leastSKUCount', 'least_package_type_id', 'leastSkuUnit', 'unitName', 'fstr_ClosingItemQty'],
            totalProperty: 'totalCount',
            root: 'data',
            autoLoad: true,
            listeners: {
    ...
    var ViewStckIndentGrid = function (fstr_id) {
        var viewTeansfer_filter = new Ext.ux.grid.GridFilters({
            filters: [{
                    type: 'string',
                    dataIndex: 'fstr_ItemName'
    ...
    var _StoreSi = function (fstr_id) {
        var _Store = new Ext.data.JsonStore({
            method: 'post',
            proxy: new Ext.data.HttpProxy({
                url: modURL + '&op=listViewStockIndentData',
                method: 'post'
            }),
            fields: ['fstr_id', 'fstrd_id', 'fstr_ItemId', 'fstr_RequiredItemQty', 'fstr_ApprovedItemQty', 'fstr_ItemName', 'subcategoryname', 'status_name', 'fstrd_status', 'category_name',
                'packageType', 'fstr_ItemUnits', 'fstr_leastSKUCount', 'least_package_type_id', 'least_package_type', 'fstr_ClosingItemQty'],
            totalProperty: 'totalCount',
            root: 'data',
            remoteSort: true,
            autoLoad: false,
    ...
    var deleteItemStockIndent = function (id) {
        Ext.MessageBox.confirm('Confirm', 'Do you want to remove this item?', function (btn, text) {
            if (btn == 'yes') {
                Ext.Ajax.request({
                    waitMsg: 'Processing',
                    method: 'POST',
                    url: modURL + '&op=deleteSIItem',
                    params: {
                        fstrd_id: id
                    },
                    success: function (response) {
                        ...
                        if (tmp.success === true) {
                            Application.example.msg('Success', 'Removed item');
                            Ext.getCmp('retalineStockIndentItemGridPanel').getStore().reload();
                        }
                    },
                    failure: function (response) {
                        var tmp = Ext.util.JSON.decode(response.responseText);
    ...
    return{
        ...
        AddNewTransferWindow: function (fstr_id, sourcename) {
            var fstr_id = fstr_id;
            var _addnewtransferWindow = new Ext.Window({
                title: 'Create Packing Order for' + sourcename,
                iconCls: 'dispatch',
                layout: 'fit',
                height: 400,
                width: 900,
        ...
        viewTransferItems: function (fstr_id) {
            ...
            _addnewtransferWindow.show();
            _addnewtransferWindow.center();
        }, initStockIndent: function () {
            var panelId = 'retalineStockIndent_main_panel';
            var branch_panel = Ext.getCmp(panelId);
            if (Ext.isEmpty(branch_panel)) {
                branch_panel = retalineStockIndentGrid(panelId);
                Application.UI.addTab(branch_panel);
                branch_panel.doLayout();
            } else {
                Application.UI.addTab(branch_panel);
            ...
            return branch_panel;
        }, viewStockIndentItems: function (fstr_id, toBranch) {
            var fstr_id = fstr_id;
            var _addnewtransferWindow = new Ext.Window({
                title: 'Stock Indent of | ' + toBranch,
                iconCls: 'dispatch',
                layout: 'fit',
                height: 400,
                width: 800,
                ...
                id: 'windowSIView',
                bodyStyle: {"background-color": "white"},
                items: [ViewStckIndentGrid(fstr_id)]
            });
            _addnewtransferWindow.doLayout();
            _addnewtransferWindow.show();
            _addnewtransferWindow.center();
