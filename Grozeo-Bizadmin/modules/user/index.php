...
switch ($op) {
    case 'setUserPassword':
        ...
        $storedPasswd = $db->getItemFromDB("SELECT Passwd FROM " . FINASCOP_DB . "finascop_usr_master WHERE UserId='{$userID}'");
        $currentPasswordValid = false;
        if (!empty($storedPasswd)) {
            if (password_verify($currentPassword, $storedPasswd)) {
                $currentPasswordValid = true;
            } elseif (strlen($storedPasswd) === 32 && md5($currentPassword) === $storedPasswd) {
                $currentPasswordValid = true;
            }
        }
        ...
        if (intval($db->getItemFromDB($qry)) > 0) {
            ...
            // TODO: Store IV alongside ciphertext and manage ENCRASS_KEY via secure key management.
            $iv = openssl_random_pseudo_bytes(16);
            $encrypted = base64_encode(openssl_encrypt($newPasswordPlain, ENCRASS_METHOD, $passkeyencrass, OPENSSL_RAW_DATA, $iv) . $iv);
            if (empty($encrypted)) {
    ...
    case 'check-current_password':
        ...
        $o->valid = false;
        $o->reason = CURR_PASS;
        if (!empty($storedPasswd) && (password_verify($currentPassword, $storedPasswd) || (strlen($storedPasswd) === 32 && md5($currentPassword) === $storedPasswd))) {
            $o->valid = true;
            unset($o->reason);
        }
        header("Content-Type: application/json");
    ...
    case 'changePassword':
        ...
        // TODO: Store IV alongside ciphertext and manage ENCRASS_KEY via secure key management.
        $iv = openssl_random_pseudo_bytes(16);
        $Encrass = base64_encode(openssl_encrypt($newPasswordPlain, ENCRASS_METHOD, $passkeyencrass, OPENSSL_RAW_DATA, $iv) . $iv);
        if (empty($Encrass)) {
            echo "{errors: { reason: 'Encrass is not enabled' }}";
            exit;
        }
