...
namespace Controllers{
	...
	class RequestHandler extends RequestAbstract{
		...
		public function __construct($request, $origin) {
                    file_put_contents('php://stderr', "CALLed RequestHandler " . false);
				file_put_contents('php://stderr', print_r($request,TRUE));
                                file_put_contents('php://stderr', print_r($origin,TRUE));
			$this->sapi_type = php_sapi_name();
			if ( $this->sapi_type == 'cli' ){
				return;
			}
			parent::__construct($request);			
			if($this->endpoint=='qugeoregistration' || $this->endpoint=='partner'){
		 ...
		 protected function partner() {
			$partner = new Partner($this->method . '_' . $this->verb);
			return $partner->{$this->method . '_' . $this->verb}($this->args,$this->request);
		 }
		 //Schedluer
