...
Ext.ux.GMapPanel = Ext.extend(Ext.Panel, {
      ...
      if (typeof GOOGLE_MAP_API_KEY === "undefined") {
        ...
        this.buildScriptTag(
          "http://maps.google.com/maps/api/js?key=" +
            GOOGLE_MAP_API_KEY +
            "&sensor=false&callback=gmapapiready"
        );
      }
    }
  },
    ...
    if (this.rendered) {
      (function () {
        ...
        google.maps.event.addListenerOnce(
          this.getMap(),
          "tilesloaded",
          this.onMapReady.createDelegate(this)
        );
        google.maps.event.addListener(
          this.getMap(),
          "dragend",
          this.dragEnd.createDelegate(this)
        );
        //added by vishnu
        this.gmap.setOptions({ scaleControl: this.scaleControl });
        ...
        if (typeof this.setCenter === "object") {
          ...
          if (this.gmapType === "panorama") {
            this.getMap().setLocationAndPOV(
              new google.maps.LatLng(this.setCenter.lat, this.setCenter.lng),
   ...
   */
  addMarkers: function (markers, autofitNpan, bounds) {
    var newmarkers = [];
    if (typeof autofitNpan === "undefined") {
      autofitNpan = false;
    }
    if (typeof bounds === "undefined") {
      bounds = { value: null };
      bounds.value = new google.maps.LatLngBounds();
    } else if (bounds.value == null) {
      bounds.value = new google.maps.LatLngBounds();
    }
      ...
      for (var i = 0; i < markers.length; i++) {
        if (markers[i]) {
          if (typeof markers[i].geoCodeAddr == "string") {
            ...
            var mkr_point = new google.maps.LatLng(
              markers[i].lat,
              markers[i].lng
            );
            ...
            bounds.value.extend(mkr_point);
          }
        }
      }
      if (autofitNpan) {
        this.getMap().fitBounds(bounds.value);
        this.getMap().panToBounds(bounds.value);
      }
    }
    return newmarkers;
  },
  ...
  addMarker: function (
    ...
    if (typeof extraInfo === "undefined") {
      ...
      mark.set("extraInfo", extraInfo);
    }

    if (typeof animate === "undefined") {
    } else {
      mark = this.animateMarker(mark, animate, animatestart);
    }
   ...
   * @param {Text} color of the marker
   */
  getDotMarkerIcon: function (color) {
    switch (color) {
      case "red":
        return "https://storage.googleapis.com/support-kms-prod/SNP_2752125_en_v0";
        break;
      case "blue":
        return "https://storage.googleapis.com/support-kms-prod/SNP_2752068_en_v0";
        break;
      case "pink":
        return "https://storage.googleapis.com/support-kms-prod/SNP_2752264_en_v0";
        break;
      case "yellow":
   ...
   * Animate marker.
   * @param {Array} marker
   * @param {Object} animation type
   * @param {Object} startend animation
   */
  animateMarker: function (marker, animationtype, startend) {
  ...
  addPolyline: function (
    ...
    if (typeof hidemarkers === "undefined") {
      nomarkers = false;
    } else {
      nomarkers = hidemarkers;
    }
    if (typeof autofitNpan === "undefined") {
      autofitNpan = false;
    }
    ...
    Ext.each(
      points,
      function (point) {
        var mkr_point = new google.maps.LatLng(point.lat, point.lng);
        plinepnts.push(mkr_point);
        bounds.value.extend(mkr_point);
        if (nomarkers == false) {
          var mark = new google.maps.Marker(
            Ext.apply(point, {
          ...
          if (typeof markericon === "undefined") {
            mark.setIcon(
              "http://maps.google.co.uk/intl/en_ALL/mapfiles/ms/micons/green-dot.png"
            );
          } else {
    ...
    return { markers: newmarkers, polyline: pline };
  },
  /**
  ...
  geoCodeLookup: function (addr, marker, clear, center, listeners) {
    ...
    this.geocoder.geocode(
      ...
      this.addAddressToMap.createDelegate(
        this,
        [addr, marker, clear, center, listeners],
        true
      )
    );
  },
  ...
  addAddressToMap: function (
    ...
    if (!response || status !== "OK") {
      ...
      var place = response[0].geometry.location,
        accuracy = this.getLocationTypeInfo(
          response[0].geometry.location_type,
          "level"
        ),
  ...
  /**
   * Fits the map to the marker.
   */
  fitBounds: function (latlngbounds) {
    this.gmap.fitBounds(latlngbounds);
  },
  /**
   * Pans the map to the center of LatlanBound
   */
  ...
  addScaleControl: function () {
    this.gmap.setOptions({ scaleControl: true });
  },
  /**
   * add google map directions
   **/
  addDirections: function (orgs, dests, waypts, hide_icon, arrows) {
  ...
  removeDirections: function (directionDisplay) {
    directionDisplay.setMap(null);
  },
  setRadius: function(markers, span, areaLatitude, areaLongitude) {
        console.log("setRadius");
        var sunCircle, mkr_point, cityCircle;
        var newmarkers = [];

        // Remove existing circles
        if (this.circles.length > 0) {
            for (var i = 0; i < this.circles.length; i++) {
                this.circles[i].setMap(null); // Remove from map
            }
        }
        this.circles = []; // Clear the circles array

        var lat = parseFloat(areaLatitude);
        var lng = parseFloat(areaLongitude);
        ...
        mkr_point = new google.maps.LatLng(lat, lng);
        ...
        this.circles.push(cityCircle); // Add the new circle to the circles array
    },drawCircles: function (circleData,panelElementId) {
      ...
      // Validate the input
      if (typeof circleData !== "object" || circleData === null) {
          console.error("circleData must be a valid object");
          return;
      }
      var panelElement = document.getElementById(panelElementId);
      if (!panelElement) {
          console.error("Invalid panel element ID:", panelElementId);
          return;
      }
      // Iterate over the keys in circleData
      for (var areaName in circleData) {
          if (!circleData.hasOwnProperty(areaName)) continue; // Skip prototype properties
  
          var data = circleData[areaName];
  
          // Validate the data
          var center = data.center;
          var span = parseFloat(data.span);
          ...
          if (
              ...
              console.error(`Invalid data for area ${areaName}:`, data);
          ...
          sunCircle = {
              ...
              fillColor: "#FF0000",
              fillOpacity: 0.35,
              map: this.getMap(), // Ensure a valid map instance
              center: { lat: center.lat, lng: center.lng },
              radius: span * 1000, // Scale span to radius in meters
          };
  
          // Create and add the new circle
          cityCircle = new google.maps.Circle(sunCircle);
          this.circles.push(cityCircle); // Store the circle for future reference
           // Create a styled InfoWindow for the area name
        /*var infoContent = `
        <div style="font-family: Arial, sans-serif; color: #333; padding: 10px; border-radius: 8px; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.2); background: #fff; max-width: 200px;">
            <div style="font-size: 16px; font-weight: bold; color: #4CAF50;">${areaName}</div>
            <div style="margin-top: 5px; font-size: 14px; color: #666;">
                <strong>Area Span:</strong> ${span}
            </div>
        </div>`;
          // Create an InfoWindow for the area name
        infoWindow = new google.maps.InfoWindow({
          content: infoContent,
          position: { lat: center.lat, lng: center.lng },
      });

      // Display the InfoWindow when the map is loaded
      infoWindow.open(this.getMap());*/
       // Add a click event listener to the circle
       google.maps.event.addListener(cityCircle, "click", (function (areaName, data) {
        return function () {
            // Update the panel with the area's data
            panelElement.innerHTML = `
                <div style="font-family: Arial, sans-serif; padding: 50px;">                    
                    ...
                    <p><strong>Latitude:</strong> ${data.center.lat}</p>
                    <p><strong>Longitude:</strong> ${data.center.lng}</p>
                    <p><strong>Population:</strong> ${data.span}</p>
                </div>
            `;
        };
    })(areaName, data));
      ...
      console.log(`${this.circles.length} circles added.`);
  },  
  /**
   * Get gmap
   */
