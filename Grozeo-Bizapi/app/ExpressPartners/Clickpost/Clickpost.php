...
class Clickpost
{
	...
	public function webhook($data)
	{
		try
        {
            ...
            (new ShippingLogRepository)->createLog([
                ...
                'APIName'       => "webhook",
                "APIURL"        => "",
				"APIHeaders"    => json_encode($data->header()),
				'request'       => json_encode($data->all()),
                'response'      => ""
            ]);
            $setTracking = $this->updateTrackingStatus($data, $ship, $data->waybill);
            return $data->delivery_id;
        }
        catch (\Exception $e)
        {
            info("CLICKPOST WEBHOOK ERROR");info($e);
        }
	}


	private function updateTrackingStatus($currentTracking, $ship, $trackingID)
	{
    ...
    {
		$timeStamp = new DateTime($currentTracking->timestamp);
		$orderDate = $timeStamp->format('Y-m-d H:i:s');
        $request = [
            "orderDate"     => $orderDate,
            "orderID"       => $ship->order_id,
            "deliveryID"    => $trackingID,
	...
	{
		$timeStamp = new DateTime($currentTracking->timestamp);
        $statDate = $timeStamp->format('Y-m-d H:i:s');
        $orderID = $ship->order_id;
        ...
        if($order)
        {
            $orderHistory = OrderHistory::create([
                ...
                'order_action'  => "Order Cancelled via Clickpost {$trackingID} on {$statDate}",
