...
{
    public partial class DeliveryBookingFailed : Base.BasePartnerPage
    {
...
left JOIN 
    retaline_package_master rpckm ON tod.rtopd_packaging = rpckm.rpckm_id
WHERE 
    co.order_id =@orderID;";
            ...
            if (custItemTbl != null && custItemTbl.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in custItemTbl.Rows)
                {
                    ...
                    string packetLength = GetSafeString(row["rpckm_length"]);
                    string packetBreadth = GetSafeString(row["rpckm_breadth"]);
                    string packetHeight = GetSafeString(row["rpckm_height"]);
