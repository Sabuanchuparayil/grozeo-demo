<%@ Page Language="C#" MasterPageFile="~/Business/BusinessMaster.master" Title="Retailer Leads" AutoEventWireup="true" CodeBehind="WholesalerLeads.aspx.cs" Inherits="RetalineProAgent.WholesalerLeads" %>
...
<asp:Content ContentPlaceHolderID="cpNBreadcrumb" runat="server">
    ...
    <li class="breadcrumb-item active" aria-current="page">Wholesaler Leads</li>
</asp:Content>
<asp:Content ContentPlaceHolderID="cpNTitle" runat="server" ID="cTitle">
    <h6 class="slim-pagetitle"><asp:Literal ID="ltrTitle1" runat="server" Text="Wholesaler Leads"></asp:Literal> 
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                                <asp:GridView AutoGenerateColumns="false" ID="gvWholesalerLeads" runat="server" CssClass="table table-bordered" GridLines="None" BorderColor="#ECECEC"
                                    AllowPaging="true" AllowSorting="true" ShowFooter="false" PagerSettings-Visible="true" PageSize="10" OnDataBound="gvWholesalerLeads_DataBound" DataSourceID="SDSWholesalerLeads">
                                    <Columns>
                                        ...
                                        <%--<asp:HyperLinkField runat="server" Text="Edit" HeaderStyle-BackColor="#DEE2E6" ItemStyle-BackColor="White" NavigateUrl="~/Business/WLeadSettings" DataNavigateUrlFields="id" DataNavigateUrlFormatString="~/Business/WLeadSettings?id={0}" />--%>
                                    </Columns>
                                    <EmptyDataTemplate>
                                        No wholsaler leads.
                                    </EmptyDataTemplate>
                                ...
                                <asp:SqlDataSource runat="server" ID="SDSWholesalerLeads" ProviderName="MySql.Data.MySqlClient" ConnectionString="<%$ ConnectionStrings:mySqlConnection %>"
                                 SelectCommand = "SELECT cl.id, ba.baName, crle_orgName,crle_location, crle_indContactperson, crle_orgContactNo, crle_CreatedBy,
                                    ...
                                    WHERE cl.crmuId NOT IN (3,7) AND crle_type=3 AND ((@areaId > 0 and cl.areaId = @areaId) or cl.baId=@baId)
                                    AND (trim(ifnull(@searchKey, '')) like '' or crle_orgName like CONCAT('%', @searchKey, '%') or crle_gplace like CONCAT('%', @searchKey, '%') or crle_indMobile like CONCAT('%', @searchKey, '%')) ORDER BY id DESC" OnSelecting="SDSWholesalerLeads_Selecting">
                            <SelectParameters>
