<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DailyITTDSReport.aspx.cs" MasterPageFile="~/Finance/FinanceMaster.master" Inherits="RetalineProAgent.Finance.DailyITTDSReport" %>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                        ...
                        <asp:GridView AutoGenerateColumns="false" DataSourceID="SDSSalesReports" ID="gvdailySalesReport" runat="server" CssClass="table table-bordered gridview_table" GridLines="None" BorderColor="#ECECEC"
                            AllowPaging="true" AllowSorting="true"  ShowFooter="false" PagerSettings-Visible="true" PageSize="10">
                             <Columns>
                                ...
                                <asp:BoundField HeaderText="No of Invoice" DataField="invoicenumber" SortExpression="invoicenumber"  ItemStyle-HorizontalAlign="Right" />
                ...
                SelectCommand="SELECT invoicedate,COUNT(invoicedate) invoicenumber,store_group_name,gst,SUM(TDS194O_IT_Total) AS TDS194O_IT_Total,SUM(amount) AS amount  FROM(
SELECT
    inv.inv_number AS invoice,invoice_type,status_id,
    DATE (inv.created_at) AS invoicedate,
...
FROM
    ...
    INNER JOIN finascop_branch_group fb ON rc.storegroup_id = fb.store_group_id WHERE invoice_type NOT IN (1,2) 
UNION ALL
SELECT
    CAST(order_invoiceno AS CHAR) AS invoice,invoice_type,status_id,
    DATE (order_invoicedate) AS invoicedate,
     (SELECT br_GST FROM `finascop_branch` WHERE br_storeGroup = fb.store_group_id LIMIT 1) AS gst,
     fb.store_group_name AS store_group_name,
     fa.TDS194O_IT_Total AS TDS194O_IT_Total,
...
FROM
    ...
    INNER JOIN `invoice_number` inv ON rc.order_id = inv.order_id
    INNER JOIN `finance_autoposting_values` fa ON fa.order_id = rc.order_id
    INNER JOIN finascop_branch_group fb ON rc.storegroup_id = fb.store_group_id) AS t WHERE invoice_type IN (1,2) AND status_id=18 AND
                                 ( @datefilter = 1 AND (@fromDate IS NULL OR @fromDate = '' OR CAST(invoicedate AS DATE) >= CAST(@fromDate AS DATE)) AND (@toDate IS NULL OR @toDate = '' OR CAST(invoicedate AS DATE) <= CAST(@toDate AS DATE))) OR ( @datefilter = 3 AND DATE_FORMAT(invoicedate, '%Y-%m') = DATE_FORMAT(CURRENT_DATE - INTERVAL 1 MONTH, '%Y-%m')
                                    ) OR ( @datefilter = 2 AND DATE_FORMAT(invoicedate, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')) GROUP BY invoicedate
