<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DetailedGSTTCSReport.aspx.cs" MasterPageFile="~/Finance/FinanceMaster.master" Inherits="RetalineProAgent.Finance.DetailedGSTTCSReport" %>
...
<asp:Content ContentPlaceHolderID="cpNTitle" runat="server">
    <h6 class="slim-pagetitle">Detailed GST TCS Report</h6>
    <p class="mb-0">Detailed GST TCS Report</p>
</asp:Content>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                        ...
                        <asp:GridView AutoGenerateColumns="false" DataSourceID="SDSSalesReports" ID="gvdailySalesReport" runat="server" CssClass="table table-bordered gridview_table" GridLines="None" BorderColor="#ECECEC"
                            AllowPaging="true" AllowSorting="true"  ShowFooter="false" PagerSettings-Visible="true" PageSize="10">
                             <Columns>
                                ...
                                <asp:BoundField HeaderText="GST & Cess (INR)" DataField="gsttota"  SortExpression="gsttota" ItemStyle-HorizontalAlign="Right" />  
                                ...
                                <asp:BoundField HeaderText="TCS" DataField="tcs" DataFormatString="{0:n}" SortExpression="tcs"  ItemStyle-HorizontalAlign="Right" /> 
                ...
                SelectCommand="    SELECT order_invoiceno,fb.store_group_name,order_invoicedate, MerchantSalesTotal_ForSettlement,(SELECT br_GST FROM `finascop_branch` WHERE br_storeGroup = fb.store_group_id LIMIT 1) AS gst,
                                (MerchantSalesIGSTTotal_ForSettlement+MerchantSalesCGSTTotal_ForSettlement+
                                MerchantSalesSGSTTotal_ForSettlement+MerchantSalesUTGSTTotal_ForSettlement+
                                MerchantSalesCCTotal_ForSettlement) AS gsttota,(COALESCE(TCSIGST,0)+ COALESCE(TCSCGST,0)+ COALESCE(TCSSGST,0)+ COALESCE(TCSCC,0)) AS tcs,
                                (MerchantSalesIGSTTotal_ForSettlement+MerchantSalesCGSTTotal_ForSettlement+
                                MerchantSalesSGSTTotal_ForSettlement+MerchantSalesUTGSTTotal_ForSettlement+
                                ...
                                INNER JOIN `invoice_number` inv ON rc.order_id = inv.order_id WHERE invoice_type=1 AND status_id=18 AND
