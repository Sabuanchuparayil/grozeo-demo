<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Finance/FinanceMaster.master" CodeBehind="Ordercalculation.aspx.cs" Inherits="RetalineProAgent.Finance.Ordercalculation" %>
        ...
        <div class="col-12">
            ...
            <div class="col-12">
                ...
                <asp:SqlDataSource runat="server" ID="SDSpendingentries" ProviderName="MySql.Data.MySqlClient"  ConnectionString="<%$ ConnectionStrings:mySqlConnection %>"
                    SelectCommand="SELECT fc.id,rc.order_order_id,fc.order_id,fc.createdOn,total,br_storeGroup,rc.order_confirmed_on,
                           (SELECT store_group_name FROM finascop_branch_group fb WHERE fb.store_group_id=br_storeGroup) AS storegroupname
                            FROM finance_autoposting_values fc 
                         ...
                         WHERE  status_id>4 and (@fromDate IS NULL OR @fromDate = '' OR CAST(rc.order_confirmed_on AS DATE) >= CAST(@fromDate AS DATE)) AND (@toDate IS NULL OR @toDate = '' OR CAST(rc.order_confirmed_on AS DATE) <= CAST(@toDate AS DATE)) and  (trim(@search) like '' or rc.order_order_id like CONCAT('%', @search, '%')) ORDER BY rc.order_order_id DESC ">
                    <SelectParameters>    
