...
{
    public partial class Ordercalculation : System.Web.UI.Page
    {
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
           ...
           if(IsPostBack && !String.IsNullOrEmpty(hidValueHeadOrderId.Value))
                ValueHeadInfo(Convert.ToInt32(hidValueHeadOrderId.Value));
        }
        ...
        protected void btnaction_Click(object sender, EventArgs e) {
            ...
            // LoadOrderValueHeads(Id);
        }

        /// <summary>
        /// Load order value heads popup
        /// </summary>
        /// <param name="Id">Order id</param>
        /// <param name="eventId">Event id (Checkout, Order Success, Packing, etc.)</param>
        /// <param name="recordType">Record Type (0: All, 1: Allocation, 2: Posting, 3: Entry)</param>
        ...
        public List<ColumnData> LoadOrderValueHeads(int Id=0, int eventId=0, int recordType=0, int ruleId=0)
        {
            ...
            if (recordType != 3)
            {
                ...
                string Columnname = "SELECT fc.* FROM finance_autoposting_values fc where fc.order_id=@order_id";
                ...
                if (Columvalues != null && Columvalues.Rows.Count > 0)
                {
                    ...
                    var orderLedger = dtColumns.AsEnumerable().Select(r=> new {fieldName= r["column_name"].ToString(), 
                        ledgerid = r["ledger_id"].ToString(), drname = r["dr_name"].ToString(), 
                        crname = r["cr_name"].ToString(), coltype = r["type"].ToString() }).ToList();

                    DataRow row = Columvalues.Rows[0];
                    foreach (DataColumn column in Columvalues.Columns)
                    {
                        string columnName = column.ColumnName;
                        if (columnName.ToLower() == "id" || columnName.ToLower() == "order_id" || !orderLedger.Any(v=> v.fieldName == columnName))
                            continue;

                        string columnValue = (row[columnName] == DBNull.Value ? "" : (row[columnName].ToString()));
                        double cval = 0; try { cval = Convert.ToDouble(row[columnName]); } catch { cval = 0; }//(row[columnName] == DBNull.Value ? " " : (row[columnName].ToString()));
                        if (!(eventId == -1 && recordType == -1) && cval == 0)
                            continue;
                        ...
                        columnDataList.Add(new ColumnData
                        {
                            ...
                            splitdrvalue = (orderLedger.Any(v => v.drname == columnName) ? columnValue : ""),
                            splitcrvalue = (orderLedger.Any(v => v.crname == columnName) ? columnValue : ""),
        ...
        private void ValueHeadInfo(int Id)
        {
            ...
            string details = "SELECT fc.id,rc.order_order_id,fc.order_id,createdOn,total,rc.order_confirmed_on,br_storeGroup,(SELECT store_group_name FROM finascop_branch_group fb WHERE fb.store_group_id=br_storeGroup) AS storegroupname,CASE WHEN baType=1 AND baMode=1 THEN 'Direct Area Associate' WHEN baType=1 AND baMode=2 THEN 'Partner Area Associate'WHEN  baType=2 AND baMode=1 THEN 'Direct Business Associate' " +
                ...
                $"WHEN payment_mode = 5 THEN 'Online With Wallet'  WHEN payment_mode = 6 THEN 'Online On Delivery' WHEN payment_mode = 7 THEN 'Cash On Delivery' END AS paymentcode,CASE WHEN order_method = 1 THEN 'Direct Delivery' WHEN order_method = 2 THEN 'Customer Collect' WHEN order_method = 3 THEN 'Courier Delivery' END AS order_method FROM finance_autoposting_values fc " +
                $"INNER JOIN retaline_customer_order rc  ON  rc.order_id=fc.order_id  INNER JOIN finascop_branch fb ON rc.order_branch_id=fb.br_id LEFT JOIN area_entries ae ON fb.areaId=ae.id LEFT JOIN business_associate ba ON ae.areaBusinessAssociate =ba.id where fc.order_id=@order_id";
        ...
        public string GetEventOnHead(string head)
        {
            ...
            string strEvents = String.Join(",",
                dtEvents.AsEnumerable().Where(r => r["column_name"].ToString() == head)
        ...
        public string GetOrdervalue(string head)
        {
            ...
            string strEvents = String.Join(",",
                dthead.AsEnumerable().Where(r => r["column_name"].ToString() == head)
        ...
        public string Getordertype(string head)
        {
            if (dttype == null)
            {
                dttype = DataServiceMySql.GetDataTable("SELECT id,column_name,TYPE FROM finance_calculation_heads ");
            }                          
            string strcost = String.Join(",",
                dttype.AsEnumerable().Where(r =>r["column_name"].ToString() == head)
