<%@ Page Language="C#" Async="true" AutoEventWireup="true" MasterPageFile="~/Tenant/TenantMaster.master" Title="Delivery Rules" CodeBehind="DeliveryRulesNew.aspx.cs" Inherits="RetalineProAgent.Tenant.DeliveryRulesNew" %>

<%@ Register Src="~/Controls/ctrlDeliveryRule.ascx" TagPrefix="uc1" TagName="ctrlDeliveryRule" %>


<asp:Content ContentPlaceHolderID="cpBreadcrumb" runat="server">
    <a href="/Tenant/Branches?type=DeliveryRates"><i class="fa fa-reply mr-2" aria-hidden="true"></i>Back</a>
</asp:Content>
                  ...
                  <div class="type_of_delivery d-flex flex-wrap p-3 mb-3 card-body share_delivery_type">
                    ...
                    <div class="deliveryTitle d-flex w-100">
                      <label class="ckbox w-auto">
                        <input id="chk_share_delivery" runat="server" class="chk check_delivery_share" type="checkbox"><span
                              ...
                              <div class="slablist p-1 p-sm-2 border mb-1 slablist-row" slabtype="2">
                                <div class="d-flex align-items-baseline align-items-sm-center flex-wrap flex-sm-nowrap">
                                  <div class="d-flex align-items-center mr-2 mr-sm-3 mb-0 mb-sm-0">
                                    <select class="form-control wd-70 mr-1 dynamic-type dynamic-input" tabindex="-1" disabled="disabled">
...
<script type="text/javascript">
    ...
    //$('#modaldelisettingspopup').on('hidden.bs.modal', function (e) {
    $('#modaldelisettingspopup_save').on('click', function (e) {
        ...
        var hidid = $('#modaldelisettingspopup_save').attr('hidid');

        var customValidator = $('#' + hidid).parent().find('.customvalidator');
        ...
        if (!isValid) {
            if (isEmpty) {
                alert('You have not added any rule for this zone. All zone values are mandatory in order to save the zone rate');
                //$('#' + hidid).val('');
                //$('a[hidid="' + hidid + '"]').removeClass('btn-primary').addClass('btn-light');
                //ValidatorValidate(customValidator[0]);
        ...
        if (hidval != '') {
            var hidvalFiltered = jQuery.grep(hidval.split('|'), function (value) {
                return !value.match("^Above");
            });

            hidval = hidvalFiltered.join('|');
            var aboveKgInput = $(aboveKg).val(); var aboveKgValInput = $(aboveKgVal).val();
            if (aboveKgInput != '' && aboveKgValInput != '') { }
    ...
    function loadDynamicValList(valueRow, slabtype) {
        ...
        var seltype = $(valueRow).find('.dynamic-type');
        var kminput = $(valueRow).find('input.dynamic-km');
        var valinput = $(valueRow).find('input.dynamic-val');
        ...
        if (slabtype == "2") {
            $(seltype).find(":selected").text(($(valueRow).closest('.slablist_wrap').find('.slablist-item').length > 0 ? 'Next' : 'First'));
            $(seltype).find(":selected").val(($(valueRow).closest('.slablist_wrap').find('.slablist-item').length > 0 ? 'Next' : 'First'));
        ...
        var hidVal = $('#' + hidid).val();
        if (hidVal && hidVal != '' && hidVal.length > 3 && !hidid.match("|$"))
            $('#' + hidid).val(hidVal + '|');

        if (slabtype == "2") {
            $(seltype).find(":selected").text('Next');
            $(seltype).find(":selected").val('Next');
        }
        $(kminput).val(''); $(valinput).val('');
...
<script type="text/javascript">
    $('.check_delivery_type').on('change', function () {
        if ($(this).is(':checked')) {
            $(this).closest('.type_of_delivery').find('div.deliveryDteails').removeClass('hide');
            ...
            $(this).closest('.type_of_delivery').find('div.showinfo').addClass('hide'); 

        }
        else {
            $(this).closest('.type_of_delivery').find('div.deliveryDteails').addClass('hide');
            ...
            $(this).closest('.type_of_delivery').find('div.showinfo').removeClass('hide'); 
        }

    });
    $('.check_delivery_share').on('change', function () {
        if (!$(this).is(':checked')) {
