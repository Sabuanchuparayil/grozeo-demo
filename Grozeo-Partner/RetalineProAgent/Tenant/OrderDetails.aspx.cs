...
{
    public partial class OrderDetails: Base.BasePartnerPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //    $"fsto_ismanualpacking,CASE WHEN fsto_ismanualpacking = 0 THEN 'Packsure App' WHEN fsto_ismanualpacking = 1 THEN 'Manual Packing' END AS manualPacking,fsto_manualpackinguserid,fsto_assigned_boy,CASE WHEN fsto_ordertype = 0 THEN(SELECT br_Name " +
            ...
            string strLtrOrdNo = "", strltrOrdDate = "", strltrSchOpnTime = "";
            string fsid = trTransferOrder["fstr_id"].ToString();
            if (sql != null)
            {
                string sqlorder = $"SELECT order_order_id, order_confirm_date, order_confirmed_on,DATE_FORMAT(order_slot_date,'%d-%m-%Y') AS slotDate," +
            ...
            //ltrPackedBy.Text = trTransferOrder["packingUserId"].ToString() + " " + "/" + " " + trTransferOrder["packingType"].ToString();
            ...
            if (tblItems2 != null && tblItems2.Rows.Count > 0)
            {
                ...
                //}
                //else if ((new int[] { 9, 12, 22, 27, 28, 29, 30, 31, 32, 33, 34 }).Contains(Convert.ToInt32(trOrder["StatusId"])))
                //{
                ...
                if (qugeoRefNo == strLtrOrdNo)
                {
                    ...
                    if (tblItems3 != null && tblItems3.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            //ltrTrackingUrl.Text = "Not Updated";
                        }
                        ...
                        if (typeName == "Courier")
                        {
                            ...
                            if (courierTbl != null && courierTbl.Rows.Count > 0)
                            {
                                ...
                                //if (ty["qoc_trackingUrl"].ToString() != "")
                                //{
                                //    ltrTrackingUrl.Text = ty["qoc_trackingUrl"].ToString();
                                //}
                                //else
                                //{
                                //    ltrTrackingUrl.Text = "Tracking URL is not updated";
                                //}
                            }
                        }
                        else if (typeName != "")
                        {
                            ...
                            //ltrTrackingUrl.Text = "This order deliver is through drive";
                        }
