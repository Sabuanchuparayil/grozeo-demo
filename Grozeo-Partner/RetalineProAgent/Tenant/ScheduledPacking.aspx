<%@ Page Language="C#" AutoEventWireup="true" Title="Scheduled Packing" MasterPageFile="~/Tenant/TenantMaster.master" Async="true"  CodeBehind="ScheduledPacking.aspx.cs" Inherits="RetalineProAgent.ScheduledPacking" %>

<asp:Content ContentPlaceHolderID="cpBreadcrumb" runat="server">
    <li class="breadcrumb-item"><a href="/">Home</a></li>
    <li class="breadcrumb-item"><a href="/navigations/PackingDelivery">Packing & Delivery</a></li>
    <li class="breadcrumb-item"><a href="/navigations/Packing">Packing</a></li>
    <li class="breadcrumb-item active" aria-current="page">Scheduled Packing</li>
</asp:Content>
<asp:Content ContentPlaceHolderID="cpTitle" runat="server">
    <h6 class="slim-pagetitle"><asp:Literal ID="ltrTitle" runat="server" Text="Scheduled Packing & Delivery"></asp:Literal></h6>
    <style>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                  <ul class="navbar-nav mr-auto">
                      ...
                      <li class="nav-item active mx-1">
                        <asp:LinkButton ID="lbtSchedulePacking" runat="server" typeid="11" OnClick="btnFilterType_Click" CssClass="nav-link btn btn-block btn-outline-primary btn-sm p-1 px-2">Scheduled Packing <span class="sr-only">(current)</span></asp:LinkButton>
                    </li>
 ...
 <div class="col-4 col-lg-2">
          ...
          <div id="accordion" class="table-responsive" style="margin-top: 8px;">
                   <asp:HiddenField ID="hidFilterType" runat="server" />
                                <asp:GridView AutoGenerateColumns="false" ID="gvPendingOrders" GridLines="None" OnRowDataBound="gvPendingOrders_RowDataBound" runat="server" CssClass="table table-bordered" BorderColor="#ECECEC"
                        ...
                        <asp:Image runat="server" ID="image" ImageAlign="AbsMiddle" width="30" ImageUrl="https://grozeo.azurewebsites.net/images/processing.gif" 
                            Visible='<%# ( (new int[]{5, 11 }).Contains(Convert.ToInt32(Eval("StatusId"))) ? true: false) %>' />
                                    ...
                                    ? true: false) %>' NavigateUrl='<%# String.Format("/ViewAndUpdate.aspx?orderid={0}&toid={1}&ordId={2}", Eval("fsto_id"), Eval("fsto_uid"), Eval("order_id")) %>' Text="View & Update Orders"></asp:HyperLink>
                                                ...
                                                <asp:Button runat="server" ID="btnMoveToPacking" OnClick="btnMoveToPacking_Click" CssClass="btn btn-primary ml-1 mr-1 mt-3 mb-3 p-1 px-3" Text="Move to Pending Orders" transferOrderId='<%# Eval("fsto_id") %>' statusId='<%# Eval("fsto_status") %>' orderType='<%# Eval("fsto_ordertype") %>' request='<%# Eval("fstr_id") %>' Visible='<%# ( (new int[]{25}).Contains(Convert.ToInt32(Eval("StatusId")))
                                    ? true: false) %>' OnClientClick="javascript:return confirm('Do you wish to move this order to pending order?');"/>
                                ...
                                <asp:SqlDataSource runat="server" ID="SDSPendingOrders" ProviderName="MySql.Data.MySqlClient" OnSelected="SDSPendingOrders_Selected" ConnectionString="<%$ ConnectionStrings:mySqlConnection %>"
                                 SelectCommand = "SELECT DISTINCT bco.order_id,bco.order_order_id,bco.order_slot_date,bco.order_total_amount,order_packedbags_count,bco.order_customer_id,order_branch_type_id,br_Lat,br_Lng,br_ID,
                                                  ...
                                                  WHERE bco.status_id > 0 AND bco.status_id >= 4 and (ifnull(@filterType, 0) = 0 
                                                     or (@filterType = 1 and bco.status_id NOT IN(17,18,19,23,24,25,34))  
                                                     ...
                                                     or (@filterType = 7 and bco.status_id IN(10,7)) or (@filterType = 8 and bco.status_id IN(16)) or (@filterType = 9 and bco.status_id IN(19, 24)) or(@filterType = 10 and bco.status_id IN(23)) or(@filterType = 11 and bco.status_id IN(25)) or(@filterType = 12 and bco.status_id IN(9) and bco.order_slot_id > 0)
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#modaldemo4').on('hidden.bs.modal', function (e) {
                window.location.href = "/Tenant/ScheduledPacking";
            });
        });
    </script>
