...
namespace Retaline.Core.Services.ScheduleTasks
{
    /// <summary>
    ...
    public partial class TaskScheduler : ITaskScheduler
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            var tenant = await _storeContext.GetCurrentTenantAsync();
            string url = "https://grozeo.in";
            if (tenant != null && tenant.Hostnames?.Length > 0)
                url = tenant.Hostnames[0];

            var scheduleTaskUrl = $"{url.TrimEnd('/')}/{RetalineTaskDefaults.ScheduleTaskPath}";
            var timeout = ScheduleTaskRunTimeout;
        ...
        protected partial class TaskThread : IDisposable
        {
            ...
            private async Task RunAsync()
            {
                ...
                try
                {
                    ...
                    client = HttpClientFactory.CreateClient(DefaultHttpClient);
                ...
                catch (Exception ex)
                {
                    ...
                    var tenant = await storeContext.GetCurrentTenantAsync();

                    message = string.Format("ScheduleTasks.Error", _scheduleTask.Name,
                        message, _scheduleTask.Type, tenant?.Name + "_" + tenant?.Id, _scheduleTaskUrl);
