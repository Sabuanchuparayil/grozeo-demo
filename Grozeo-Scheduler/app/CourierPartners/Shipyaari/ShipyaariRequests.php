            ...
            if($package->fsto_status == 10)
            {
                ...
                if($productData)
                {
                    ...
                    if(@$orderData->order_branch_id > 0)
                    {
                        ...
                        {
                            $store_addr1 = (@$branchData->br_Address != "") ? [$branchData->br_Address] : [@$branchData->br_City, @$branchData->district->dst_Name, @$branchData->state->st_name];
                            ...
                            $data['from_details'] = [
                                ...
                                'address1'          => implode(', ', array_filter($store_addr1)),
                            ...
                            foreach ($orderData->orderItems as $oitem)
                            {
                                $data['package_details']['products'][] = [
                                    ...
                                    "weight"			=> (@$oitem->item->stit_courierWt ?? 0),
                                    "line_item"			=> 0
