...
use App\Finance\India\FinanceIndiaFunctions;
...
//info("mathExpression: {$mathExpression} order_id : {$order_id}"); 
    ...
    private function getMathExpression($order_id,$valueHeadFormula){
        ...
        if(substr( $valueHeadFormula, 0, 3) === "FN_"){
            ...
            if (method_exists(FinanceIndiaFunctions::class, $functionName)) {
                //info("Method $functionName calling.");
                $head_value = FinanceIndiaFunctions::$functionName($order);
        ...
        if($order)
        {
            ...
            if($fExists == 0)
            {
                FinanceAutopostingValues::create([
                    ...
                    'RetailSalePriceinMRP'      => $order->order_mrp_et,
                    'MRP_RRP'                   => $order->order_mrp,
                    'TaxinMRP'                  => ($order->order_mrp - $order->order_mrp_et)
                ]);
            }
        }
    }
    private function orderSuccess($data)
    {
        ...
        if($order)
        {
            ...
            {
                $autoPostingVals['CGSTonSSM'] = round($order->order_sales_margin * 0.09, 2);
                $autoPostingVals['SGSTonSSM'] = round($order->order_sales_margin * 0.09, 2);
            }
            else
            {
                $autoPostingVals['IGSTonSSM'] = round($order->order_sales_margin * 0.18, 2);
            }
            $customer = Customer::where('cust_id', $order->order_customer_id)->first();
            if($customer->primaryAddress->state->st_ID == $goozeoState[0]->cfg_Value)
            {
                $tRCGST = ($order->order_delivery_charge_cgst > 0) ? $order->order_delivery_charge_cgst : ($order->order_delivery_charge_igst/2);
                $tRSGST = ($order->order_delivery_charge_sgst > 0) ? $order->order_delivery_charge_sgst : ($order->order_delivery_charge_igst/2);
                $autoPostingVals['CGSTonODC'] = $tRCGST;
                $autoPostingVals['SGSTonODC'] = $tRSGST;
            }
            else
            {
                $tRIGST = ($order->order_delivery_charge_igst > 0) ? $order->order_delivery_charge_igst : ($order->order_delivery_charge_cgst + $order->order_delivery_charge_sgst);
                $autoPostingVals['IGSTonODC'] = $tRIGST;
            }
            ...
            else if(($order->payment_mode == 3) || ($order->payment_mode == 5))
            {
            ...
            foreach ($order->orderItems as $item)
            {
                ...
                {
                    $autoPostingVals['RetailSalePrice_Restaurant'] += ($item->order_item_mrp_et - $item->order_item_seller_discount);
                    $autoPostingVals['IGSTonRSP_Restaurant'] += $item->order_item_igst;
                    $autoPostingVals['CGSTonRSP_Restaurant'] += $item->order_item_cgst;
                    $autoPostingVals['SGSTonRSP_Restaurant'] += $item->order_item_sgst;
                    $autoPostingVals['UTGSTonRSP_Restaurant'] += $item->order_item_ugst;
                    $autoPostingVals['CConRSP_Restaurant'] += $item->order_item_cess;
                    $autoPostingVals['GSTonRSP_RestaurantTotal'] += $item->order_item_gst;
                }
                else
                {
                    $autoPostingVals['RetailSalePrice'] += ($item->order_item_mrp_et - $item->order_item_seller_discount);
                    $autoPostingVals['IGSTonRSP_Final'] += $item->order_item_igst;
                    $autoPostingVals['CGSTonRSP_Final'] += $item->order_item_cgst;
                    $autoPostingVals['SGSTonRSP_Final'] += $item->order_item_sgst;
                    $autoPostingVals['UTGSTonRSP_Final'] += $item->order_item_ugst;
                    $autoPostingVals['CConRSP_Final'] += $item->order_item_cess;
                    $autoPostingVals['TCSIGST'] += $item->order_item_tcs_igst;
                    $autoPostingVals['TCSCGST'] += $item->order_item_tcs_cgst;
                    $autoPostingVals['TCSSGST'] += $item->order_item_tcs_sgst;
                    $autoPostingVals['TCSUTGST'] += $item->order_item_tcs_utgst;
                    $autoPostingVals['TCSGSTTotal'] += $item->order_item_tcs_gst;
                    $gstOthers += $item->order_item_gst;

                }
            }
            if($autoPostingVals['RetailSalePrice_Restaurant'] > 0)
            {
                $restTot = $autoPostingVals['RetailSalePrice_Restaurant'] + $autoPostingVals['GSTonRSP_RestaurantTotal'];
                $roundRestTot = round($restTot);
                $restTotRound = round(($restTot - $roundRestTot), 2);
                $restTotRoundABS = abs($restTotRound);
                if($restTotRound > 0)
                {
                    $autoPostingVals['RoundDown_RestaurantSales'] = abs($restTotRound);
                    $autoPostingVals['RoundUp_RestaurantSales'] = 0;
                }
                if($restTotRound < 0)
                {
                    $autoPostingVals['RoundUp_RestaurantSales'] = abs($restTotRound);
                    $autoPostingVals['RoundDown_RestaurantSales'] = 0;
                }
            }
            if($autoPostingVals['RetailSalePrice'] > 0)
            {
                $otherTot = $autoPostingVals['RetailSalePrice'] + $gstOthers + $autoPostingVals['CConRSP_Final'];
            ...
            if($order->payment_mode == 2 || $order->payment_mode == 3 || $order->payment_mode == 5)
            {
                ...
                {
                    $autoPostingVals['CGSTInputonMDR'] = $order->order_payment_gateway_tax/2;
                    $autoPostingVals['SGSTInputonMDR'] = $order->order_payment_gateway_tax/2;
                }
                else
                {
                    $autoPostingVals['IGSTInputonMDR'] = $order->order_payment_gateway_tax;
                }
                ...
                if($branchData->br_State == $goozeoState[0]->cfg_Value)
                {
                    if($autoPostingVals['SellerMDR_SMDR'])
                    {
                        $autoPostingVals['CGSTonSMDR'] = round($autoPostingVals['SellerMDR_SMDR'] * 0.09, 2);
                        $autoPostingVals['SGSTonSMDR'] = round($autoPostingVals['SellerMDR_SMDR'] * 0.09, 2);
                    }
                }
                else
                {
                    if($autoPostingVals['SellerMDR_SMDR'])
                    {
                        $autoPostingVals['IGSTonSMDR'] = round($autoPostingVals['SellerMDR_SMDR'] * 0.18, 2);
                    }
                }
                $autoPostingVals['OrderGrandTotal'] = $order->total;
                $autoPostingVals['OrderGrandTotal_POD'] = 0;
            }
            else if($order->payment_mode == 1 || $order->payment_mode == 4)
            {
                $autoPostingVals['OrderGrandTotal'] = 0;
                $autoPostingVals['OrderGrandTotal_POD'] = $order->total;
                $autoPostingVals['TSOPOD_PendingCollection'] = $order->total - $order->order_wallet_amount;
            }
