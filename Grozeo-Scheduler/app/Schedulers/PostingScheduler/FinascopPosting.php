...
final class EntryTypes {
    const Ledger = 0;
    const SellerRetailers = 1;
    const DynamicCostCentre = 2;
    const AreaAssociates = 3;
    const BusinessAssociates = 4;
    const DeliveryAgents = 5;
    const BusinessPartner = 6;
}
...
class FinascopPosting
{
    ...
    public function finascopPosting($order_id, $finascopEventRefId, $storeGroupId)
    {
        try
        {
            ...
            if ($event) {
                  $query = "SELECT id, rulename FROM finance_autoposting_rule WHERE event_master_id = {$event->id} AND status = 1";
        ...
        {
            info("Finascop Posting ruleDetails finascopPosting error: . ".$e->getMessage());
    ...
    private function finascopPostingRun($order_id, $eventID, $finascopEvent, $ruleName,$ruleID, $storeGroupId = -1)
    {
        ...
        try
        {
            ...
            //info("rule_id : {$rule[0]->rule_id} Rule Name : {$rule[0]->rulename}");
            ...
            $voucher = [
                'finascopBrID'        => 1, //it is hard coded as per current requirements and will be changed when branch is implemented.
            ...
            if(count($ruleDetails) > 0)
            {
                foreach ($ruleDetails as $rd)
                {
                    ...
                    try{
                        ...
                        $amount = DB::table('finascop_autoposting_calculations as fac')
                        //->join('finance_calculation_heads as fch', 'fac.head_id', '=', 'fch.id')
                        ->where('fac.order_id', $order_id)
                        ->where('fac.head_id', $rd->value_head_id)
                        ->selectRaw('ROUND(fac.head_value, 2) as rounded_value')
                        ->value('rounded_value');

                    }
                    catch (\Exception $cnEx)
                    {
                        info("Finascop Posting ruleDetails finascopPostingRun. ".$cnEx->getMessage());
                        $logCNF = DB::table('finascop_log')->insertGetId([
                            'entity_id' => $order_id,'createdOn' => date('Y-m-d H:i:s'),'type' => $finascopEvent,'status' => 0,
                            'comments' => $rd->column_name . ' Value not found in finascop_autoposting_calculations table.  Rule : ' . $ruleName . ' on Order number : ' . $order_number . '.  Applying rule_id : ' . $ruleID
                        ]);
                    }
                    ...
                    {
                        switch($rd->type_id){
                            case EntryTypes::SellerRetailers :
                                
                                $refId = $storeDetails->storeRefId;
                                break;
                            case EntryTypes::DeliveryAgents:
                                $refId = DB::table('retaline_customer_order as rto')
                                ->join('qugeo_order as qo', 'rto.order_order_id', '=', 'qo.quor_RefNo')
                                ->join('qugeo_driver as qd', 'qo.quor_DeliveryDriverId', '=', 'qd.d_id')
                                ->where('qo.quor_RefNo', $order_number)
                                ->value('ref_id');
                                break;
                            case EntryTypes::BusinessAssociates:
                                $refId = $this->getBusinessAssociateRefId($order_id);
                                break;
                            case EntryTypes::AreaAssociates:
                                $refId = $this->getAreaAssociateRefId($order_id);
                                break;
                            case EntryTypes::BusinessPartner:
                                $refId = $this->getBusinessPartnerRefId($order_id);
                                break;
                        }
                    }
                    catch (\Exception $ex)
                    {
                        info("Finascop Posting finascopPostingRun. 400 ".$ex->getMessage());
                    }
                    if ($rd->isDebtor == 1)
                    {
                        $voucher['Account'][] = [
        ...
        {
            info("Finascop Posting finascopPostingRun. 401 ".$e->getMessage());
        ...
        catch (Exception $ex)
        {
            ...
            return new ErrorWithData('Exception : Finascop Posting FinascopPostingRun. 402 ' . $ex->getMessage(), $voucher, 406);
        }

        if (!is_object($response) || !isset($response->statusId)) {
             DB::table('finascop_log')->where('id', $logRecID)->update(
            ['status' => 2,'comments' =>  'Invalid Response from finascop server: ' .$response . ' on Event : ' .$finascopEvent . ' rule : ' . $ruleName . '.  Applying rule_id : ' . $ruleID . ' on Order number : ' . $order_number . ' Status: ' . json_encode($response) . 'URL: ' . $url . ' headers: ' . implode(',', $headers) . 'Data: ' . json_encode($voucher)
            ]);
            
            info("Error: " . $response);

            return new ErrorWithData("FinascopDataEntry returned with invalid response :" .$response, $voucher, 406);           
        }
    ...
    private function getOrderReferralMerchantRefId($order_id)
    {
        ...
        {
            info("Finascop Posting getOrderReferralMerchantRefId. ".$e->getMessage());
            return NULL;
        }
        return NULL;
    }
    ...
    private function getCustomerSourceMerchantRefId($order_id)
    {
        ...
        {
            info("Finascop Posting getCustomerSourceMerchantRefId. ".$e->getMessage());
            return NULL;
        }
        return NULL;
    }
    ...
    private function getBusinessAssociateRefId($order_id)
    {
        ...
        {
            info("Finascop Posting getBusinessAssociateRefId.".$e->getMessage());
    ...
    private function getAreaAssociateRefId($order_id)
    {
        ...
        {
            info("Finascop Posting getAreaAssociateRefId. ".$e->getMessage());
            return NULL;
        }
        return NULL;
    }
    ...
    private function getBusinessPartnerRefId($order_id)
    {
        ...
        {
            info("Finascop Posting getBusinessPartnerRefId. ".$e->getMessage());
            return NULL;
        }
        return NULL;
    }
    ...
    private function getCostDistributionRule($order_id, $eventID, $storeGroupId,$valueHeadID)
    {
        ...
        {
            info("Finascop Posting getCostDistributionRule. ".$e->getMessage());
