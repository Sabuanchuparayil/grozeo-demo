...
class ReAssignOrder extends OrderAssigner
{
    ...
    public function __invoke()
    {
        try
        {
            ...
            ProcessLock::updateColData("BizAPI_ReAssignOrder", 0);
        }
        catch (\Exception $e)
        {
    ...
    protected function getPendingOpenOrders()
    {
        return BoyOrderRequest::select('id', 'boy_id', 'branch_id', 'order_id','order_pk_id')
            ...
            ->whereRaw(
                '(IF(updated_at IS NOT NULL, updated_at, created_at) < ?)', 
                [now()->subMinutes(static::TIME_OUT_FREQUENCY)]
            )
            ->get();
    }
