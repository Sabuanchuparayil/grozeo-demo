...
class DeliveryNotCompletedDelay
{
    public function __invoke()
    {
        try
        {
            ...
            ->whereIn('status_id', [10, 11, 12, 13, 14, 15])
                ...
                if($getRuleData)
                {
                    ...
                    if($delType)
                    {
                        ...
                        if(@$historyCheck[0]['created_at'])
                        {
                            ...
                            if($timeDiff > $delType->deliveryCompletionTime)
                            {
            ...
            ProcessLock::updateColData("BizAPI_DeliveryNotCompletedDelay", 0);
        }
        catch (\Exception $e)
        {
            info("DeliveryNotCompletedDelay ERROR => ".$e);
            ProcessLock::updateColData("BizAPI_DeliveryNotCompletedDelay", 1);
        }
    }
    ...
    private function checkModeMethod($order)
    {
        ...
        $checkShipping = ShippingConsignment::where('order_id', $order->order_order_id)->first();
        if($checkShipping)
        {
            $outs['mode'] = 2;
            $outs['method'] = @$checkShipping->shipping_type;
        }
        return $outs;
    }
