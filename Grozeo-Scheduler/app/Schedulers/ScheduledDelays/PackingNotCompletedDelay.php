...
class PackingNotCompletedDelay
{
    public function __invoke()
    {
        try
        {
                ...
                if($getRuleData)
                {
                    ...
                    if($delType)
                    {
                        ...
                        if(@$historyCheck[0]['created_at'])
                        {
                            ...
                            if($timeDiff > $delType->packingCompletion_maxDelay)
                            {
            ...
            ProcessLock::updateColData("BizAPI_PackingNotCompletedDelay", 0);
        }
        catch (\Exception $e)
        {
            info("PackingNotCompletedDelay ERROR => ".$e);
            ProcessLock::updateColData("BizAPI_PackingNotCompletedDelay", 1);
        }
    }
