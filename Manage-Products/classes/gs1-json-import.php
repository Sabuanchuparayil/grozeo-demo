...
class Gs1JSONImport
{
	...
	{
		$catID = $_GET['catid'];
		$subcatID = (@$_GET['page']) ? $_GET['subcatid'] : 0;
		$current = (@$_GET['page']) ? $_GET['page'] : 1;
		$outs = [];

		if($subcatID == 0)
		{
			$qry = 'select id, subCategoryName from gs1_subCategory where categoryId='.$catID;
			$subCategories = $db->getMultipleData($qry, true);
			if($subCategories)
			{
				foreach ($subCategories as $subCat)
				{
					$files = glob("gs1-json/".$catID."/".$subCat['id']."/*");
					$data = $this->insertIntoProduct($catID, $subCat['id'], 1, $files, $db);
					$outs[] = [
						'CatID'		=> $catID,
						'subcatID'	=> $subCat['id'],
						'total'		=> $files,
	...
	private function insertIntoProduct($catID, $subcatID, $page, $total, $db)
	{
		$productInserts = 0;
		$mrpInserts = 0;
		for($i = $page; $i <= $total; $i++)
		{
			$jsonItems = file_get_contents("gs1-json/".$catID."/".$subcatID."/".$i.".json");
			if($jsonItems)
			{
				$products = array_map(function($item) use ($db, $catID, $subcatID)
				{
					$returns = [];
					if($item->company_detail->name)
					{
						$qry = 'select * from gs1_company where companyName = "'.$this->textFormatter($item->company_detail->name).'"';
				...
				if(count($products) > 0)
				{
					...
					foreach($products as $product)
					{
						if($product)
						{
							...
							$insertQuery = 'INSERT INTO gs1_products_json_import ('.join(', ', array_keys($product)).') VALUES ('.$productString.')';
							...
							if($mrpWithGST[$x])
							{
								...
								$pInsert = $db->perform('gs1_product_mrp_json_import', $mrpdata, 'insert');
								$mrpInserts++;
							}
						}
						$x++;
		...
		return [
			'products_inserted'	=> $productInserts,
			'mrp_inserts'		=> $mrpInserts
		];
	}
