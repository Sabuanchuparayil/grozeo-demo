...
function financialTransactionReturnSellable($trId, $lastId, $quor_TransferOrder_Type, $quor_TransferOrder_id, $quor_id) {
    global $db;
    //BEGIN FINANCIAL TRANSACTION
    //DEBIT CS_STOCK IN TRANSIT
    //CREDIT CS_STOCK


    if ($_SESSION['admin']->br_PyramidLevel == 2) { //br_PyramidLevel 1:CPD,2:Central Store,3:Distributor,4:Retailer
        $quor_Refno_Source = $db->getItemFromDB("SELECT quor_Refno_Source FROM qugeo_order WHERE quor_id = {$quor_id} ");

        $query = "SELECT waqs_id,waqs_Name,waqs_Configuration FROM finascop_wallet_queue_settings WHERE waqs_id = 79";
        $wqSettings = $db->getFromDB($query, true);
//$fields = array("tempvalue" => 'Amount:'.$amount);
//$db->perform('temptable',$fields);

        $transctionTemplate = json_decode($wqSettings['waqs_Configuration'], true);
        $transctionTemplate['cr']['csStock']['amt'] = round(doubleval($amount), 2);
        $transctionTemplate['dr']['csSales']['amt'] = round(doubleval($amount), 2);
    } else if ($_SESSION['admin']->br_PyramidLevel == 3) {
        $quor_Refno_Source = $db->getItemFromDB("SELECT quor_Refno_Source FROM qugeo_order WHERE quor_id = {$quor_id} ");
        if ($quor_Refno_Source == 2) {//0-CPDtoBR, 1-B2C, 2-B2B
            $query = "SELECT waqs_id,waqs_Name,waqs_Configuration FROM finascop_wallet_queue_settings WHERE  waqs_id = 80";
            $wqSettings = $db->getFromDB($query, true);
            $transctionTemplate = json_decode($wqSettings['waqs_Configuration'], true);
            $transctionTemplate['cr']['distStock']['amt'] = round(doubleval($amount), 2);
            $transctionTemplate['dr']['distSales']['amt'] = round(doubleval($amount), 2);
        }
    }
    ...
    $commentsamt = round(doubleval($amount), 2);
    $search = array("#ID#", "#AMT#");
    $replace = array($quor_RefNo, $commentsamt);
...
function financialTransactionCustPickup($trId, $lastId, $quor_TransferOrder_Type, $quor_TransferOrder_id, $quor_id) {
    global $db;
    //BEGIN FINANCIAL TRANSACTION
    //DEBIT CS_STOCK IN TRANSIT
    //CREDIT CS_STOCK
...
function financialTransaction($trId, $lastId, $quor_TransferOrder_Type, $quor_TransferOrder_id, $quor_id) {
    global $db;
...
//           $db->perform('temptable',$fields);
    ...
    if ($_SESSION['admin']->br_PyramidLevel == 2) { //br_PyramidLevel 1:CPD,2:Central Store,3:Distributor,4:Retailer
        ...
        $query = "SELECT waqs_id,waqs_Name,waqs_Configuration FROM finascop_wallet_queue_settings WHERE waqs_id = 8";
        $wqSettings = $db->getFromDB($query, true);
//$fields = array("tempvalue" => 'Amount:'.$amount);
//$db->perform('temptable',$fields);
    ...
    $commentsamt = round(doubleval($amount), 2);
    $search = array("#AMT#", "#ID#");
    $replace = array($commentsamt, $quor_RefNo);
    $transctionTemplate['comments'] = str_replace($search, $replace, $transctionTemplate['comments']);
...
class cgoS3FileHandler {
    ...
    function putFileToS3($S3Object, $S3Bucket, $FilePath, $file_name) {
