...
class AccountingVouchers {
    ...
    private function updateLedgerBalance($acet_NO, $add_amount) {
        global $db;
        $query = "SELECT actr_amount, actr_IsDebtor,ledg_Id FROM " . FINASCOP_DB . "finascop_accounts_transaction WHERE acet_NO = '{$acet_NO}'";
        $rd = $db->getMultipleData($query, true);
        foreach ($rd as $k => $value) {
            if ($value['actr_IsDebtor'] == 0) {
                if ($add_amount == 1) {
                    $qry = "UPDATE " . FINASCOP_DB . "finascop_accounts_ledger as al SET accled_Credits = accled_Credits + {$value['actr_amount']}  WHERE al.accled_Ledger_Id = {$value['ledg_Id']}";
                } elseif ($add_amount == 0) {
                    $qry = "UPDATE " . FINASCOP_DB . "finascop_accounts_ledger as al SET accled_Credits = accled_Credits - {$value['actr_amount']}  WHERE al.accled_Ledger_Id = {$value['ledg_Id']}";
                }
                $db->query($qry);
                ...
                if ($add_amount == 1) {
                    $qry = "UPDATE " . FINASCOP_DB . "finascop_accounts_ledger as al SET accled_Debits = accled_Debits + {$value['actr_amount']}  WHERE al.accled_Ledger_Id = {$value['ledg_Id']}";
                } elseif ($add_amount == 0) {
                    $qry = "UPDATE " . FINASCOP_DB . "finascop_accounts_ledger as al SET accled_Debits = accled_Debits - {$value['actr_amount']}  WHERE al.accled_Ledger_Id = {$value['ledg_Id']}";
                }
                $db->query($qry);
...
//           $db->perform('temptable',$fields);          
    ...
    public function updatedate($data, $sessiondets) {
        ...
        $updated_on = sha1(microtime(true) . mt_rand(10000, 90000));
        $updated_by = $sessiondets->Finascop_UserId;
        $data = array('acet_ApprovedBy' => $sessiondets->Finascop_UserId, 'acet_Date' => $data['receipt_date'], 'updated_on' => $updated_on, 'updated_by' => $updated_by);
        $status = $db->perform(FINASCOP_DB . "finascop_accounts_entry", $data, "update", "acet_NO = '{$acet_NO}'");
        //Save  Transaction
        $accounts_transaction_data = array('actr_Date' => $data['receipt_date']);
        $db->perform(FINASCOP_DB . "finascop_accounts_transaction", $accounts_transaction_data, "update", "acet_NO = '{$acet_NO}'");
        $msg = "Updated date";
        return '{"success" : true,"msg":"' . $msg . '","updated_on":"' . $updated_on . '"}';
    }
