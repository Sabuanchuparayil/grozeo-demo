...
function user_access($module, $operation = DEFAULT_OPERATION, $account = NULL) {
    ...
    $capability = "";
    if (empty($operation)) {
        $qry = "select if(coalesce(Capability,'')='',0,OperationId) as Capability from sys_module_operation where ModuleName=?";
        $rows = $db->getMultipleSafe($qry, "s", [$module], true);
    } else {
        $qry = "select if(coalesce(Capability,'')='',0,OperationId) as Capability from sys_module_operation where ModuleName=? and Operation=?";
        $rows = $db->getMultipleSafe($qry, "ss", [$module, $operation], true);
    }

    $capability = NULL;
    if (!empty($rows)) {
        foreach ($rows as $tmp) {
            $capability = is_array($tmp) ? $tmp['Capability'] : $tmp;
...
function blocked_capabilities($accountID) {
    ...
    if (count($blocked_capabilities) > 0 && $blocked_capabilities[0] !== '') {
        $blockedList = "'" . join("','", $blocked_capabilities) . "'";
        $qry = "SELECT MenuId FROM sys_module_operation WHERE OperationId in ($blockedList) AND (capability IS NOT NULL OR capability<>'')";
