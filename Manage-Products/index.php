...
if (PHP_VERSION_ID >= 70300) {
    ...
    session_set_cookie_params(0, $sessionCookieParams['path'] . '; samesite=Strict', $sessionCookieParams['domain'], $sessionCookieParams['secure'], true);
} elseif (function_exists('ini_set')) {
    ini_set('session.cookie_lifetime', '0');
    ini_set('session.cookie_path', dirname($_SERVER['PHP_SELF']));
    ini_set('session.cookie_domain', $_SERVER['SERVER_NAME']);
    ini_set('session.cookie_httponly', '1');
    ini_set('session.cookie_secure', $sessionCookieParams['secure'] ? '1' : '0');
}
session_start();
...
//Import GET/POST variables — access $_GET/$_POST directly in modules (no extract).

//Set Requested Module, to Execute that requested on
if (isset($_GET['module']))
    $module = preg_replace('/[^a-z0-9_]/', '', $_GET['module']);
if (isset($_POST['module']))
    $module = preg_replace('/[^a-z0-9_]/', '', $_POST['module']);

$allowedModules = array_map('basename', glob(ROOT . '/modules/*', GLOB_ONLYDIR));
...
if (isset($_SESSION['admin'])) {
    ...
    if ($module == 'email_action' || $module == 'vblogin') {
