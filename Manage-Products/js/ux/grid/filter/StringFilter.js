Ext.ux.grid.filter.StringFilter = Ext.extend(Ext.ux.grid.filter.Filter, {
	...
	init: function(config){
		...
		this.menu.add(value);
		this.config = Ext.applyIf(config,{minChars: 0});
		
		this.updateTask = new Ext.util.DelayedTask(this.fireUpdate, this);
	},
	
	onKeyUp: function(event){
	    console.dir([this.value.getValue().length, this.config.minChars]);
	...
	isActivatable: function(){
		return this.value.getValue().length > this.config.minChars;
	},
