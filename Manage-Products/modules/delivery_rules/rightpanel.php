...
if ($rdr_id > 0) {
    ...
    $data = $db->getFromDB("SELECT rdr_id,rdr_ruleName,rdr_deliveryMode,rdr_calculationMode,rdr_ruleFor,"
            ...
            WHEN rdr_deliveryMode = 3 THEN 'Group Delivery'
        ...
        rdr_isfreeDeliveryAmt,IF(rdr_isfreeDelivery = 1,rdr_isfreeDeliveryAmt,0) AS freeDelivery, rdr_ruleFor,rdr_ruleForId,is_default FROM retaline_delivery_rules WHERE rdr_id =' " . $rdr_id . "'", true);
}?>

<html><style>

        .cesstable{ border-collapse:collapse;}
    .cesstable td,.cesstable th{border-bottom:1px solid #EDEDED; font-family: Source Sans Pro,Verdana,Geneva,sans-serif;
                                                  font-size: 12px;padding:3px;}
    .cesstable th{ padding-right:21px; padding-left:14px;width:35%; vertical-align:top; font-weight: inherit; text-align: left;}
    .cesstable td{font-weight:bold; }
    .cesstable td span{float:left; width:95%; text-align:justify; font-weight:normal;}
    .cesstable th[colspan="2"]{font-size:13px;  background: #f0f0f0 none repeat scroll 0 0;
                                        border-color: #d7d7d7;}
    </style>
<?php if (!empty($data)) { ?>
    ...
    <?php if ($data['rdr_calculationMode'] == 2) { ?>
                    <tr>
                        ...
                        <td>
        <?php echo $data['rdr_fixedRateperkm']; ?>
                        </td>
                    </tr>
                    <tr>
                        <th width="40%" text-align="left">
