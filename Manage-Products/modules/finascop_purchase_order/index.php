...
//       exit();
        ...
        $adhocCount = $db->getItemSafe("SELECT COUNT(*) FROM finascop_purchase_order_poadhoc WHERE adhoc_uniqueid = ? AND adhoc_updatedon = '{$_POST['poadhoc_updatedon']}'", "s", [$_POST['fpot_uniqueid']]);
...
//        }
 ...
 FROM finascop_purchase_order_poadhoc fp  {$filter_qry} where adhoc_uniqueid = ?", "s", [$_POST['uniqueid']], true);
        if (!empty($podata)) {
//            $fpot_assigneddby = $db->getItemSafe("SELECT fpot_assigneddby FROM finascop_purchase_order_temp WHERE fpot_uniqueid = ? GROUP BY fpot_uniqueid", "s", [$_POST['uniqueid']]);
...
//            }
    ...
    case 'deleteAdhocDetails':
        $db->query('begin');
        $temp = $db->executeSafe("DELETE FROM finascop_purchase_order_poadhoc WHERE adhoc_uniqueid = ?", "s", [$_POST['adhoc_uniqueid']]);
        if ($temp) {
            $db->executeSafe("DELETE FROM finascop_purchase_order_temp WHERE fpot_uniqueid = ?", "s", [$_POST['adhoc_uniqueid']]);
        }
        $status = $db->query('commit');
        if ($status) {
