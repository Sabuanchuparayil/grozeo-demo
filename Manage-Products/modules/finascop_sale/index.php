    ...
    case 'saveSalesReturn':
        ...
        foreach ($sales_return_gridData as $key => $val) {
            ...
            if ($val['return_qty'] != 0) {
                ...
                $qry = "UPDATE  " . FINASCOP_DB . "finascop_sales_invoice_items SET saii_returnedQty = saii_returnedQty + {$return_quantity}, saii_resaleableQty = saii_resaleableQty + {$resaleable_qty}  WHERE saen_Id = '{$_POST['saen_id']}' AND saii_itemID = '{$val['itemid']}'";
                $status = $db->query($qry);
            }
        }
