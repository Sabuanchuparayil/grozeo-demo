...
//        med_filter.autoReload = true;
    ...
    var manufacturePanel = function (id) {
        var _manufacturePanel = new Ext.Panel({
            ...
            items: [
                manufactureGrid(),
                new Ext.Panel({
                    title: 'Manufacturer/Suppliers Details',
                    frame: false,
                    border: true,
                    region: 'east',
    ...
    var saveManufacture = function () {
        ...
        var params = {
            action: 'Save Manufacture/Supplier',
            module: 'Master',
            op: 'saveManufacture',
    ...
    var unitValue = function(){
        ...
        var _unitValueGrid = new Ext.grid.EditorGridPanel({
      ...
      if (unitId > 0 && !Ext.isEmpty(unitValue)) {
        ...
        Ext.Msg.alert("Notification", "Please enter required fields");
          ...
          listeners: {
            ...
            afteredit: updateUnitValue
          },
        });
        return _unitValueGrid;
      };

      function updateUnitValue(grid_event) {
        var rec = grid_event.record;
        Application.MyphaMaster.unitValue = rec.data.value;
        Application.MyphaMaster.unitValueId = (!Ext.isEmpty(rec.data.id)) ? rec.data.id : 0;
        var unitId = Ext.getCmp('gridpanelMasterDataviewUnitdata').getSelectionModel().getSelections()[0].data.unit_id;

        if (unitId > 0 && !Ext.isEmpty(rec.data.value)) {
            Ext.Ajax.request({
              url: modURL + "&op=saveUnitValue",
              params: {
                unitValueId:Application.MyphaMaster.unitValueId,
                  unitId: unitId,
                  unitValue: Application.MyphaMaster.unitValue
              },
              failure: function (response) {
                Ext.MessageBox.alert("Error", response.responseText);
              },
