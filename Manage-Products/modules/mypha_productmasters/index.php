    ...
    case 'listSubcategory':
        ...
        $filter_qry .= buildSafeFilterQuery($data['filter'], $allowedFields, $db);
                    ...
                    if ($field['data']['value'] == 'Active') {
                        $fiterItem = 1;
                        $searchitem .= " and ( sub.status = {$fiterItem}) ";
                    } else if ($field['data']['value'] == 'Inactive') {
                        $fiterItem = 0;
                        $searchitem .= " and (sub.status = {$fiterItem}) ";
                    } else {
                        $searchitem .= " and (sub.status = 1 or sub.status=0) ";
                    }
                } else {
                    $checkComa = strstr($field['data']['value'], ',');
                    ...
                    if ($checkComa != '') {
                        ...
                        $searchDataValue = mysqli_real_escape_string($db->linker(), $field['data']['value']);
                        $searchitem .= " and ({$field['field']} LIKE '{$searchDataValue}%') ";
                    }
                }
            }
        }
