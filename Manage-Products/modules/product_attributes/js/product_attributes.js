Application.ProductAttributes = (function () {
  ...
  var gridSelectionChangedattributeValue = function () {
    if (
      ...
      var ID = Ext.getCmp("gridpanelMasterDataviewAttributeValuedata")
       ...
       var attributeMasterName =  Ext.getCmp("gridpanelMasterDataviewAttributeValuedata")
        .getSelectionModel()
        .getSelections()[0].data.name;
        Application.ProductAttributes.Cache.attributeId =ID; 
        Application.ProductAttributes.Cache.attributeMasterName =attributeMasterName; 
  ...
  var AttributeMasterMasterStore = function () {
    var _attributeMasterMasterStore = new Ext.data.GroupingStore({
      ...
      reader: new Ext.data.JsonReader(
        ...
        ["id", "name", "status","subCategory","category_name","parent_category","business_type_name"]
      ),
      sortInfo: {
        field: "id",
        direction: "DESC",
  ...
  var AttributeValueStore = function () {
    var _attributeMasterMasterStore = new Ext.data.GroupingStore({
      ...
      reader: new Ext.data.JsonReader(
        ...
        ["id", "name", "valueCount","subCategory","business_type_name","parent_category","category_name"]
      ),
      sortInfo: {
        field: "id",
        direction: "DESC",
    ...
    var _attributeValuemaingridPanel = new Ext.grid.GridPanel({
      ...
      listeners: {
        cellClick: function (grid, rowIndex, columnIndex, e) {
          var record = grid.getStore().getAt(rowIndex);
          var ID = record.get("id");
          var attributeMasterName = record.get("name");
          if (!Ext.isEmpty(ID)) {
            Application.ProductAttributes.Cache.attributeId = ID;
            Application.ProductAttributes.Cache.attributeMasterName = attributeMasterName;
      ...
      tbar: [
        {
          ...
          handler: function () {
            ...
            Ext.getCmp('valueName').enable();
            Ext.getCmp('attributes').enable();
            Ext.getCmp('attributes').trigger.hide();
            Ext.getCmp('attributes').setValue(Application.ProductAttributes.Cache.attributeId);
            Ext.getCmp('attributes').setRawValue(Application.ProductAttributes.Cache.attributeMasterName);
            
            Ext.getCmp('addValueBtn').enable();
            Ext.getCmp("gridAttributeValueList")
                .getStore()
                .load({
                  params: {
                    attributeId: Application.ProductAttributes.Cache.attributeId,
                  },
                });
  ...
  var attributeValueSubGrid = function () {
    ...
    _attributeValueGridFilter.remote = true;
    _attributeValueGridFilter.autoReload = true;
    var _attributeValuemaingridPanel = new Ext.grid.EditorGridPanel({
      region: "center",
      layout: "fit",
          ...
          items: [
            {
              ...
              handler: function (grid, rowIndex, colIndex, itm, evn) {
                ...
                Ext.Ajax.request({
                  ...
                  url: modURL,
                  params: {
                    op: "removeAttributeValue",
                    attributeValueId: record.get("id"),
                  },
      ...
      listeners: {
        afteredit: function (grid_event) {
          updateAttributeValue(grid_event);
        }
      }
    });
    return _attributeValuemaingridPanel;
  };
  function updateAttributeValue(grid_event, labId) {
    var data = Ext.encode(grid_event.record.data);
    Ext.Ajax.request({
      waitMsg: "Please wait...",
      url: modURL + "&op=updateAttributeValue",
      params: {
  ...
  return {
    ...
    ViewAttributeMaster: function () {
      var id = arguments[0];
      /*<?php if (user_access("product_attributes", "saveAttributeMaster")) { ?> */
      Ext.getCmp("buttonMasterAttributeMasterEdit").show();
    ...
    ViewAttributeValue: function (attributeId) {
      ...
      Ext.Ajax.request({
        ...
        success: function (response) {
          ...
          Ext.getCmp("buttonAttributeValueEdit").show();
