...
function loadInTransitDetails() {
    ...
    $ind = intval($_POST['ind']);
    if (!empty($br_id)) {
        $qry = "SELECT COUNT(*) FROM " . FINASCOP_DB . "qugeo_order " . " INNER JOIN qugeo_deliverystatus ON dls_ID = quor_Status WHERE  ((quor_type in (2,3,4) and quor_Status=9 ) or (quor_type=1 and quor_Status in (23,24,25,26,27,28,29,32,33,34,35,36,37,38)) ) and quor_Deliverybr_id = {$br_id} ";
        ...
        $query = "SELECT quor_RefNo as booking_no,DATE_FORMAT(quor_Date,'%d-%m-%Y') as booked_at,"
                ...
                . " WHERE ((quor_type in (2,3,4) and quor_Status=9 ) or (quor_type=1 and quor_Status in (9,10,11,12,13,14,23,24,25,26,27,28,29,32,33,34,35,36,37,38)) ) and quor_Pickupbr_id = {$br_id} {$search} "
