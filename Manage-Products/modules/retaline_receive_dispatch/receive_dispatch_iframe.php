    ...
    <!--                    <td>
                ...
                for ($i = 0; $i < count($countDetails); $i++) {
                    if ($countDetails[$i]['fsto_ItemId'] > 0) {
                        $itemDetails = $db->getFromDb("SELECT fsto_ItemId,(SELECT stit_SKU FROM finascop_stock_itemmaster where stit_ID = fsto_ItemId) as item_name,fsto_ItemQty,fstro_receivedOn,fstro_receivedTime FROM finascop_stock_transfer_order_details WHERE fsto_id = {$fsto_id} AND fsto_ItemId = {$countDetails[$i]['fsto_ItemId']}", true);
