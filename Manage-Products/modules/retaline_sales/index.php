...
switch ($op) {
    ...
    case 'getB2BCustomerItems' :
        $query = $_POST['query'] ?? '';
        if (!empty($query) && $query != 'WHERE 1 = 1') {
            $con = "WHERE stit_SKU LIKE '%" . $db->sanitizeString($query) . "%' AND stit_SalesEnabled = 1";
        } else {
            $con = "WHERE stit_SalesEnabled = 1";
        }
        $qry = "select stit_ID,stit_SKU from " . FINASCOP_DB . "finascop_stock_itemmaster {$con} ORDER BY stit_SKU";
