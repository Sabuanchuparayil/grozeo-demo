...
switch ($op) {
    case 'sendMailFromQueue':
        ...
        $rd = $db->getMultipleSafe('select l.EmailLogId,l.Recipient,l.Subject,c.Content, c.Attachments from sys_email_log l, sys_email_log_content c where l.EmailLogId=c.EmailLogId and l.EmailLogId=? and status="Waiting"', 'i', [$logId], true);
        if (!empty($rd)) {
        foreach ($rd as $rs) {
            ...
            $db->perform('sys_email_log', $data, 'update', 'EmailLogId=' . intval($rs['EmailLogId']));
        }
        }
        break;
    case 'get-mod-menu':
