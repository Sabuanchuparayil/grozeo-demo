    ...
    class QugeoOrderHandler{
        ...
        public function AssignOrderToQugeoDriver($orderid,$apikey,$orderdetails,&$nextorder,&$isnewroute,$getOrder=true){
                ...
                if(isset($nors) && count($nors) > 0 ){
                        ...
                        $CurrentOrders = array();
                        array_push($CurrentOpenOrders,array('orderid'=>$orderid,'order'=>0,'Latitude'=>$NewOrderLat,'Longitude'=>$NewOrderLong,'IsClosed'=>0,'IsLiveOrder'=>0,'IsPickup'=>$orderdetails['IsPickup'],'order'=>QUGEO_LIVE_ORDER_INCREMENT_VALUE));
                }			
                $isnewroute =false;
                $nextorder = array();
                $db = new \cgoSqlDB();
                        ...
                        if($getOrder && $IsMilestoneLocked==0){
                                ...
                                if($NewLiveOrder!=$LiveOrder && $value['orderid']==$NewLiveOrder ){
                                        ...
                                        if($rsno['IsPickup']=='1'){
                                                ...
                                                $qugeoDetails = $db->getFromDB("SELECT quor_TransferOrder_id,quor_DeliveryMethodsAllowed,quor_Type,quor_TransferOrder_Type,quor_Deliverybr_id,"
                                                . "quor_AmountCollectible FROM qugeo_order WHERE quor_id = {$rsno['quor_id']}", true);
                                              ...
                                              $query = "SELECT waqs_id,waqs_Name,waqs_Configuration FROM finascop_wallet_queue_settings WHERE waqs_id = 8";
                                                ...
                                                $refnosourcetype = $db->getItemFromDb('select quor_Refno_Source from  qugeo_order where quor_id = ' . $rsno['quor_id']);

                                                $amt = getSourceOrderGrandtotal($refnosourcetype,$rsno['quor_RefNo']);
//                                                echo('amt:'.$amt);
//                                                exit(1);
 
            
                                                $transctionTemplate['dr']['csStockInTransit']['amt'] = $amt;
                                                $transctionTemplate['cr']['csStock']['amt'] = $amt;	

                                                $comment = str_replace('#ID#',$rsno['quor_id'],$transctionTemplate['comments']);
                                                $comment = str_replace('#NO#',$rsno['quor_RefNo'],$comment);
                                                $comment = str_replace('#AMT#',$amt,$comment);
                                                $comment = str_replace('#NAME#',$rsno['deliveryname'],$comment);								

                                                $transctionTemplate['comments'] = $comment;


                                                //$sourcetype,$rsno['quor_RefNo']
        ...
        public function GetNextOrder($apikey,&$nextorder,$readonly=false){
                $arrVehicle=array();
                $arrVehicle['PartitionKey'] = array('col'=>'apikey','val'=>$apikey,'oper'=>'=');
