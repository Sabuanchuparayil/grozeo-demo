...
Ext.ux.GMapPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent: function () {
        ...
        if (window.google && window.google.maps) {
            ...
            if (typeof GOOGLE_MAP_API_KEY === 'undefined') {
                ...
                this.buildScriptTag('http://maps.google.com/maps/api/js?key=' + GOOGLE_MAP_API_KEY + '&sensor=false&callback=gmapapiready');
            }
        }
            ...
            for (var i = 0; i < markers.length; i++) {
                if (markers[i]) {
                    if (typeof markers[i].geoCodeAddr == 'string') {
                        ...
                        var newmarker = this.addMarker(mkr_point, markers[i].marker, false, markers[i].setCenter, markers[i].listeners, markers[i].icon, markers[i].extraInfo);
     ...
     */
    addMarker: function (point, marker, clear, center, listeners, icon, extraInfo, animate, animatestart) {
     ...
     * @param {Object} linestyle an object defining the line style to use
     */
    addPolyline: function (points, linestyle, autofitNpan, bounds, icons, hidemarkers, markericon) {
        ...
        Ext.each(points, function (point) {
            ...
            bounds.value.extend(mkr_point);
            if (nomarkers == false) {
                var mark = new google.maps.Marker(Ext.apply(point, {
                    position: mkr_point, title: point.title
                }));
                if (typeof markericon === 'undefined') {
                    mark.setIcon('http://maps.google.co.uk/intl/en_ALL/mapfiles/ms/micons/green-dot.png');
                } else {
